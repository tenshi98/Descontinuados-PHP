...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			case 'idVideoCat':    if(empty($idVideoCat)){    $error['idVideoCat']    = 'error/No ha ingresado el id de la categoria';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$sql_usuario = mysql_query("SELECT Nombre FROM saludos_videos_categorias WHERE Nombre='".$Nombre."' "); 
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `saludos_videos_categorias` (Nombre, img, n_enviados) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				//Filtros
				$a = "idVideoCat='".$idVideoCat."'" ;
				if(isset($Nombre) && $Nombre != ''){                $a .= ",Nombre='".$Nombre."'" ;}
				if(isset($img) && $img != ''){                      $a .= ",img='".$img."'" ;}
				if(isset($n_enviados) && $n_enviados != ''){        $a .= ",n_enviados='".$n_enviados."'" ;}
		
				// inserto los datos de registro en la db
				$query  = "UPDATE `saludos_videos_categorias` SET ".$a." WHERE idVideoCat = '$idVideoCat'";
				$result = mysql_query($query, $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			//se borra al usuario
			$query  = "DELETE FROM `saludos_videos_categorias` WHERE idVideoCat = {$_GET['del']}";
			$result = mysql_query($query, $dbConn);	
			
			//se borran los permisos del usuario
			$query  = "DELETE FROM `saludos_videos_listado` WHERE idVideoCat = {$_GET['del']}";
			$result = mysql_query($query, $dbConn);	
...
/*******************************************************************************************************************/
		case 'del_img':	
			...
			if(unlink($arrUsuario['ex_img'].$archivo)){	
				...
				$query  = "UPDATE `saludos_videos_categorias` SET ".$a." WHERE idVideoCat = '{$_GET['del_img']}'";
				$result = mysql_query($query, $dbConn);
				//Redirijo			
