...
$original = "mantencion_tarjetas_saludos.php";
$location = $original;
...
if ( !empty($_GET['del']) )     {
	...
	$form_obligatorios = '';
	$form_trabajo= 'del';
	require_once '../XRXS_2D2CFFDJFDJX1/xrxs_form/saludos_tarjetas_enviados.php';	
}
?>
<!doctype html>
        ...
        <div class="col-lg-10 fcenter"  style="margin-bottom:15px;">
			...
			<script type="text/javascript">
                jwplayer("myElement").setup({
                    file: "<?php echo $arrUsuario['web_ex_upload'].$rowdata['VideoSaludo'] ?>",
...
$query = "SELECT idTarjetas FROM `saludos_tarjetas_enviados` ";
...
$arrVideos = array();
$query = "SELECT 
saludos_tarjetas_enviados.idTarjetas,
saludos_tarjetas_enviados.fecha_video,
postulante.PostNombres,
postulante.PostApellPapa,
postulante.PostApellMama
FROM `saludos_tarjetas_enviados`
LEFT JOIN `postulante`   ON postulante.id_usuario    = saludos_tarjetas_enviados.id_usuario
ORDER BY saludos_tarjetas_enviados.fecha_video ASC
...
<div class="col-lg-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Tarjetas Enviadas</h5>
		</header>
    ...
    <?php foreach ($arrVideos as $videos) { ?>
    	<tr class="odd">
			...
			<td>
<?php if ($rowlevel['level_editar']==1){?><a href="<?php echo $location.'&vid_id='.$videos['idTarjetas']; ?>" title="Ver Video" class="icon-editar-img info-tooltip"></a><?php } ?>
<?php if ($rowlevel['level_borrar']==1){?><?php $ubicacion=$location.'&del='.$videos['idTarjetas'];?>
