...
if ( !empty($_POST['edit_datos']) )  { 
	...
	$form_obligatorios = 'Nombre,email,Rut,fNacimiento';
	$form_trabajo= 'update';
...
if ( !empty($_POST['edit_clave']) )  { 
	...
	$location.='?next=true';
	//Llamamos al formulario
	$form_obligatorios = 'oldpassword,password,repassword';
	$form_trabajo= 'pass';
	require_once '../XRXS_2D2CFFDJFDJX1/xrxs_form/usuarios_listado.php';
}
...
if (isset($_GET['datamod'])) {$error['usuario'] 	  = 'sucess/Tus datos han sido modificados correctamente';}
if (isset($_GET['passmod']))  {$error['usuario'] 	  = 'sucess/Tu clave ha sido modificada correctamente';}
if (isset($_GET['imgmod'])) {$error['usuario'] 	      = 'sucess/Imagen modificada correctamente';}
...
// Se traen todos los datos de mi usuario
$query = "SELECT Nombre,Fono,email,Rut,fNacimiento,Ciudad,Comuna,Direccion,idTheme    
FROM `usuarios_listado`
WHERE idUsuario = {$arrUsuario['idUsuario']}";
$resultado = mysql_query ($query, $dbConn);
$rowdata = mysql_fetch_assoc ($resultado);
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post">
			...
			if(isset($idTheme)) {       $x13 = $idTheme;      }else{$x13 = $rowdata['idTheme'];}
			...
			echo form_select('Tema','idTheme', $x13, 1, 'idTheme', 'Nombre', 'core_theme_colors', 0, $dbConn);
			?> 
