...
$original = "perfil_producciones.php";
$location = $original;
...
if ( !empty($_POST['submit']) )  { 
	...
	$form_obligatorios = 'ProdDesc';
	$form_trabajo= 'insert';
	require_once '../XRXS_2D2CFFDJFDJX1/xrxs_form/perfil_producciones.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$form_obligatorios = 'idProd,ProdDesc';
	$form_trabajo= 'update';
	require_once '../XRXS_2D2CFFDJFDJX1/xrxs_form/perfil_producciones.php';
}
//se borra un dato
...
if (isset($_GET['created'])) {$error['usuario'] 	  = 'sucess/Produccion creada correctamente';}
if (isset($_GET['edited']))  {$error['usuario'] 	  = 'sucess/Produccion editada correctamente';}
if (isset($_GET['deleted'])) {$error['usuario'] 	  = 'sucess/Produccion borrada correctamente';}
//Manejador de errores
if(isset($error)&&$error!=''){echo errors_list($error);};?>
<?php ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
 if ( ! empty($_GET['id']) ) { 
// Se traen todos los datos de mi usuario
$query = "SELECT ProdDesc
FROM `producciones`
WHERE idProd = {$_GET['id']}";
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		<header>
			...
			<h5>Modificacion de la Produccion</h5>
		</header>
		...
		<form class="form-horizontal" method="post">
            ...
            if(isset($ProdDesc)) {        $x1  = $ProdDesc;       }else{$x1  = $rowdata['ProdDesc'];}

			//se dibujan los inputs
            echo form_input('text', 'Nombre', 'ProdDesc', $x1, 2); 
			?>
 ...
 <div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post">
            ...
            <?php 
			//Se verifican si existen los datos
            if(isset($ProdDesc)) {        $x1  = $ProdDesc;       }else{$x1  = '';}
...
if (isset($_GET['search'])){
	$z.="WHERE ProdDesc LIKE '%{$_GET['search']}%'";	
}
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT idProd FROM `producciones` ".$z."";
...
$query = "SELECT idProd,ProdDesc
FROM `producciones`
".$z."
ORDER BY ProdDesc ASC
...
<label class="control-label col-lg-4">Buscar Produccion</label>
    <div class="col-lg-5">
...
<?php if ($rowlevel['level_crear']==1){?><a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width" >Crear Nueva Produccion</a><?php } ?>
</div>
...
<div class="col-lg-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Producciones</h5>
		</header>
    ...
    <?php foreach ($arrDepartamento as $depto) { ?>
    	<tr class="odd">
			<td class=" "><?php echo $depto['ProdDesc']; ?></td>
			<td class=" ">
<?php if ($rowlevel['level_editar']==1){?><a href="<?php echo $location.'&id='.$depto['idProd']; ?>" title="Editar Informacion" class="icon-editar info-tooltip"></a><?php } ?>
<?php if ($rowlevel['level_borrar']==1){?><?php $ubicacion=$location.'&del='.$depto['idProd'];?>
