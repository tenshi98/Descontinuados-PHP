...
$original = "postulante_listado.php";
$location = $original;
...
$query = "SELECT PostNombres, PostApellPapa, PostApellMama, PostFecNac, PostFonoFijo, PostFonoCel, idDepartamento, idProvincia, idDistrito, PostDireccion, PostEmail, PostProfesion1, PostProfesion2, PostProfesion3, PostProduccion1, PostProduccion2, PostProduccion3, rol
FROM `postulante`
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		<header>
			...
			<h5>Modificacion datos del Postulante</h5>
		</header>
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			if(isset($PostNombres)) {         $x1  = $PostNombres;          }else{$x1  = $rowdata['PostNombres'];}
            if(isset($PostApellPapa)) {       $x2  = $PostApellPapa;        }else{$x2  = $rowdata['PostApellPapa'];}
            if(isset($PostApellMama)) {       $x3  = $PostApellMama;        }else{$x3  = $rowdata['PostApellMama'];}
			if(isset($PostFecNac)) {          $x4  = $PostFecNac;           }else{$x4  = $rowdata['PostFecNac'];}
            if(isset($PostFonoFijo)) {        $x5  = $PostFonoFijo;         }else{$x5  = $rowdata['PostFonoFijo'];}
            if(isset($PostFonoCel)) {         $x6  = $PostFonoCel;          }else{$x6  = $rowdata['PostFonoCel'];}
			if(isset($idDepartamento)) {      $x7  = $idDepartamento;       }else{$x7  = $rowdata['idDepartamento'];}	
			if(isset($idProvincia)) {         $x8  = $idProvincia;          }else{$x8  = $rowdata['idProvincia'];}
            if(isset($idDistrito)) {          $x9  = $idDistrito;           }else{$x9  = $rowdata['idDistrito'];}
			if(isset($PostDireccion)) {       $x10 = $PostDireccion;        }else{$x10 = $rowdata['PostDireccion'];}
			if(isset($PostEmail)) {           $x11 = $PostEmail;            }else{$x11 = $rowdata['PostEmail'];}
			if(isset($PostProfesion1)) {      $x12 = $PostProfesion1;       }else{$x12 = $rowdata['PostProfesion1'];}
			if(isset($PostProfesion2)) {      $x13 = $PostProfesion2;       }else{$x13 = $rowdata['PostProfesion2'];}
			if(isset($PostProfesion3)) {      $x14 = $PostProfesion3;       }else{$x14 = $rowdata['PostProfesion3'];}
			if(isset($PostProduccion1)) {     $x15 = $PostProduccion1;      }else{$x15 = $rowdata['PostProduccion1'];}
			if(isset($PostProduccion2)) {     $x16 = $PostProduccion2;      }else{$x16 = $rowdata['PostProduccion2'];}
			if(isset($PostProduccion3)) {     $x17 = $PostProduccion3;      }else{$x17 = $rowdata['PostProduccion3'];}
			if(isset($rol)) {                 $x18 = $rol;                  }else{$x18 = $rowdata['rol'];}
			...
			echo form_date('F Nacimiento','PostFecNac', $x4, 1);
			echo form_input('text', 'Telefono Fijo', 'PostFonoFijo', $x5, 1);
			echo form_input('text', 'Telefono Movil', 'PostFonoCel', $x6, 1);
			echo form_select_depend2('Departamento','idDepartamento', $x7, 1, 'idDepartamento', 'Nombre', 'ubicacion_departamento', 0,
									 ...
									 $x7,$x8,$dbConn);						 
			echo form_input('text', 'Direccion', 'PostDireccion', $x10, 1);
			echo form_input('text', 'Email', 'PostEmail', $x11, 1);
			echo form_select('Talento 1','PostProfesion1', $x12, 1, 'ProfCod', 'ProfDesc', 'profesion', 0, $dbConn);
			echo form_select('Talento 2','PostProfesion2', $x13, 1, 'ProfCod', 'ProfDesc', 'profesion', 0, $dbConn);
			echo form_select('Talento 3','PostProfesion3', $x14, 1, 'ProfCod', 'ProfDesc', 'profesion', 0, $dbConn);
			echo form_select('Produccion 1','PostProduccion1', $x15, 1, 'idProd', 'ProdDesc', 'producciones', 0, $dbConn);
			echo form_select('Produccion 2','PostProduccion2', $x16, 1, 'idProd', 'ProdDesc', 'producciones', 0, $dbConn);
			echo form_select('Produccion 3','PostProduccion3', $x17, 1, 'idProd', 'ProdDesc', 'producciones', 0, $dbConn);
			echo form_select('Rol','rol', $x18, 1, 'idRol', 'Nombre', 'postulante_roles', 0, $dbConn);					 
			?>
...
if (isset($_GET['search'])&& $_GET['search'] != ''){
	$z.=" AND postulante.PostEmail LIKE '%{$_GET['search']}%' ";	
}
...
$query = "SELECT postulante.id_usuario FROM `postulante` ".$z."";
...
<div class="col-lg-12">
    ...
    <?php foreach ($arrPostulante as $postulante) { ?>
    	<tr class="odd">
			<td><?php echo $postulante['PostEmail']; ?></td>
			<td><?php echo $postulante['PostNombres'].' '.$postulante['PostApellPapa'].' '.$postulante['PostApellMama']; ?></td>
            <td><?php echo $postulante['alias']; ?></td>
			<td><?php echo $postulante['rol_pos']; ?></td>
			<td>
<?php if ($rowlevel['level_ver']==1){?><a target="_blank" href="<?php echo 'view_postulante.php?view='.$postulante['id_usuario']; ?>" title="Visualizar" class="icon-ver info-tooltip"></a><?php } ?>
<?php if ($rowlevel['level_editar']==1){?><a href="<?php echo $location.'&id='.$postulante['id_usuario']; ?>" title="Editar Informacion" class="icon-editar info-tooltip"></a><?php } ?>
<?php if ($rowlevel['level_borrar']==1){?><?php $ubicacion=$location.'&del='.$postulante['id_usuario'];?>
