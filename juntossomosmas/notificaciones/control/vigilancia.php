...
$sql2="SELECT LEFT( fecha_hora, 10 ) as fecha FROM vigilancia order by id_sms desc LIMIT 0, 1"; 
...
$i=0;
$sql2="SELECT * FROM vigilancia where LEFT( fecha_hora, 10 )='".$fecha_hoy."' group by id_usuario";
$gral=mysql_query($sql2,$base);
while($registro=mysql_fetch_array($gral)) 
{ 
...
$i=$i+1;
 ...
 for($i1=0; $i1<$i; $i1++)
   {
//$sql2="SELECT * FROM vigilancia where  id_usuario=400 and LEFT( fecha_hora, 10 )='".$fecha_hoy."' order by fecha_hora desc "; 
$sql2="SELECT * FROM vigilancia where  id_usuario='".$usuario[$i1]."' and LEFT( fecha_hora, 10 )='".$fecha_hoy."' order by fecha_hora desc  LIMIT 0, 1"; 
$gral=mysql_query($sql2,$base);
while($registro=mysql_fetch_array($gral)) 
{ 
	...
	// LINK PARA VER EL DETALLE DE LA UBICACION
     infoWindow: {
          content: "<p><b><u><?=$registro2["nom_ejecutiv"]?></u></b></p><p><?=$registro2["cod_fono"]?>-<?=$registro2["fono_ejecutiv"]?></p>"+
		           "<p><a href='http://<?=$nombreurl?>/vigilancia/detalle.php?lon=<?=$lon?>&lat=<?=$lat?>&nom=<?=$registro2["nom_ejecutiv"]?>&fono=<?=$registro2["cod_fono"]?>-<?=$registro2["fono_ejecutiv"]?>'>Ver Ubicaci&oacute;n</a>.</p><p><a href='http://<?=$nombreurl?>/vigilancia/historico_usuario.php?usuario=<?=$registro["id_usuario"]?>&nom=<?=$registro2["nom_ejecutiv"]?>&fono=<?=$registro2["cod_fono"]?>-<?=$registro2["fono_ejecutiv"]?>'>Ver Hist&oacute;rico </a>.</p>"
        }
	// LINK PARA VER EL DETALLE DE LA UBICACION
      });
...
<table align="center" width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="581" rowspan="4" align="center" valign="middle">
        ...
        <span class="google_fjalla_gde">&Uacute;ltima Posici&oacute;n Registrada(<?=$fecha_hoy?>)</span>
	</td></tr>
</table> 
