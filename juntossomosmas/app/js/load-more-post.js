	...
	$.fn.vasplus_post_scroller = function(options) {
		...
		return this.each(function() 
		{
			...
			function vasplus_load_more_data_system() {
				if($vpb_config.vpb_url == "") { alert('Error: No hay comentarios, Gracias!'); }
				else {
			...
			if( $vpb_config.vpb_auto_load == true ) // Auto Scroll
			{
				$(window).scroll(function() {
					if($(window).scrollTop() + $(window).height() > $this.height() && !vpb_process_started) {
						...
						$this.find('#'+$vpb_config.vpb_loading_div_id).html('<center><div align="center"><img src="images/loadings.gif" align="absabsmiddle" title=" " /></div></center>');
						setTimeout(function() {
