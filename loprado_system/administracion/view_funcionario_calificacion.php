...
$original = "principal_datos_calificacion.php";
$location = $original;
$location .= '?d=d';
/**********************************************************************************************************************************/
...
PERIODO
FROM Calificacion
WHERE Id_Funcionario = {$rowdata['idCasChile']} AND Fecha_Cierre!=''
ORDER BY PERIODO DESC";
$resultado = mssql_query ($query, $sql_conection);
while ( $row = mssql_fetch_assoc ($resultado)) {
array_push( $arrCalificacion,$row );
}					
?>
	...
	<div class="box">
		<header>
			...
			<ul class="nav nav-tabs pull-right">
				...
				<li class="active"><a href="view_funcionario_calificacion.php?pagina=1&view=<?php echo $_GET['view'];?>" >Calificaciones</a></li>
			</ul>	
		</header>
        <div class="body">
			
			<div class="col-lg-12">
				<h2 class="text-primary">Calificaciones</h2>
				<div class="table-responsive">
				<table class="table">
					<tr class="info">
						<td colspan="2">Listado de Calificaciones</td>
					</tr>
					<?php foreach($arrCalificacion as $calificacion) {?>
						<tr>
							<td><?php echo 'Periodo '.$calificacion['PERIODO'] ?></td>
							<td width="120px"><a target="_blank" href="view_calificacion.php?periodo=<?php echo $calificacion['PERIODO'].'&idCasChile='.$rowdata['idCasChile']; ?>" title="Ver Informacion" class="icon-ver info-tooltip"></a></td>
						</tr>
					<?php } ?>
				</table>
				</div>	
