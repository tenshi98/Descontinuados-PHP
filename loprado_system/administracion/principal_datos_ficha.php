...
WHERE FichaFuncionario.Id_Funcionario = {$arrUsuario['idCasChile']}";
...
WHERE FichaMunicipal.Id_Funcionario = {$arrUsuario['idCasChile']}";
$resultado = mssql_query( $query, $sql_conection );
$rowlaboral = mssql_fetch_assoc ($resultado);


// Se trae un listado con todas las modificaciones solicitadas
...
WHERE usuarios_sol_mod.idUsuario = {$arrUsuario['idUsuario']}

ORDER BY usuarios_sol_mod.idEstado ASC 
LIMIT 10";
$resultado = mysql_query ($query, $dbConn);
while ( $row = mysql_fetch_assoc ($resultado)) {
array_push( $arrModificaciones,$row );
}						
?>
	...
	<div class="box">
		<header>
			...
			<ul class="nav nav-tabs pull-right">
				...
				<li class="active"><a href="principal_datos_ficha.php" >Ficha</a></li>
			...
			<div class="col-lg-4">
				...
				<div class="bs-example" style="margin-top:20px;">
					<table class="table table-hover">
						<tr class="info">
						  <td colspan="2">Solicitudes Pendientes</td>
						</tr>
						<tr class="active">
						  <td>Fecha</td>
						  <td>Estado</td>
						</tr>
						<?php foreach ($arrModificaciones as $mod) { ?>
						<tr>
						  <td><?php echo Fecha_estandar($mod['Fecha']) ?></td>
						  <td><?php echo $mod['Estado'] ?></td>
						</tr>
						<?php } ?>   
				...
				<div class="btn-group-vertical" role="group" aria-label="..." style="margin-top:10px; width:100%;">
					<a href="<?php echo $location ?>&id=true" class="btn btn-primary" >Solicitar Modificacion de Datos</a>
