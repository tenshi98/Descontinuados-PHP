...
/*******************************************************************************************************************/
	...
	if ( !empty($_POST['idCasChile']) )     $idCasChile     = $_POST['idCasChile'];
...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			...
			case 'Apellido':       if(empty($Apellido)){      $error['Apellido']       = 'error/No ha ingresado el Apellido';}break;
			case 'Rut':            if(empty($Rut)){           $error['Rut']            = 'error/No ha ingresado el Rut';}break;
			case 'idCasChile':     if(empty($idCasChile)){    $error['idCasChile']     = 'error/No ha ingresado el fNacimiento';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$sql_email = mysql_query("SELECT Nombre FROM usuarios_listado WHERE Nombre='".$Nombre."' AND idSistema='".$idSistema."' ");
			...
			// se verifica si el correo ya existe
			if(isset($Rut)){
				$sql_email = mysql_query("SELECT Rut FROM usuarios_listado WHERE Rut='".$Rut."'  AND idSistema='".$idSistema."' ");
				$n2 = mysql_num_rows($sql_email);
			} else {$n2=0;}
