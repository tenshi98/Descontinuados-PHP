...
if(isset($_GET['new_view_app'])&&$_GET['new_view_app']!=''){
...
switch ($componentes[0]['Codigo']) {
        ...
        <?php foreach ($componentes as $idcomp) { ?>
            <td rowspan="<?php echo $idcomp['body_fil']?>" colspan="<?php echo $idcomp['body_col']; $xy = $xy+$idcomp['body_col'] ?>">
                <div class="box_col_1 <?php echo $idcomp['grid_color'].' '.$idcomp['grid_border'].' '.$idcomp['grid_shadow'].' '.$idcomp['grid_img'] ?>">
					<?php echo $idcomp['idTipoBoton'] ?>
					<?php 
					//Se verifica si el usuario tiene la sesion activa
					if ( empty($arrCliente) ) {
                        $link = 'login.php'.$w.'&return_to1=principal.php';
					}elseif(!empty($arrCliente)){
						switch ($idcomp['idTipoBoton']) {
							case 0:
						$link = $idcomp['Archivo'].$w.'&idTipoAlerta='.$idcomp['idTipoAlerta'].'&contactar='.$idcomp['contactar'].'&desplegar='.$idcomp['desplegar'];		
								break;
							case 1:
  ...
  <a class="<?php echo $config_app['btn_enlace_color_fondo'].' '.$config_app['btn_enlace_ancho'].' '.$config_app['btn_enlace_radio'].' '.$config_app['btn_enlace_float'].' '.$config_app['btn_enlace_color_texto'].' '.$config_app['btn_enlace_sombra'] ?> btn_link" href="<?php echo 'login.php'.$w.'&return_to1=principal.php' ?>">Iniciar Sesion</a>
