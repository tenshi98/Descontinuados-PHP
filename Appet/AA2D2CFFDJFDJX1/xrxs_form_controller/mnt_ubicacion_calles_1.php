...
//Se validan si existe el usuario o el email asociado de este en la base de datos
	...
	if(isset($Nombre)&&isset($idCiudad)&&isset($idComuna)){
		$sql_calle = mysql_query("SELECT Nombre FROM mnt_ubicacion_calles WHERE Nombre='".$Nombre."' AND idCiudad='".$idCiudad."' AND idComuna='".$idComuna."'"); 
		$n_com = mysql_num_rows($sql_calle);
	} else {$n_com=0;}
...
//Validaciones de ingreso de datos obligatorios
	...
	if ( empty($Nombre) )      $errors[4] 	  = '
	<div id="txf_04" class="alert_error">  
	  	No ha ingresado un nombre para la calle
