...
<?php
$busca = mssql_query("SELECT TOP 1 RUT FROM MAESTRO_CONTRIBUYENTE WITH (NOLOCK) WHERE RUT='".$rut."' OR RUT='".$rut1."' OR RUT='".$rut2."'", $lnk_patcom);
...
if ($haysocial == 1){
 ...
 while($row = mssql_fetch_array($result)) {
	...
	while($row03 = mssql_fetch_array($sql03)) {
			 ...
			 $sql_mlb2="SELECT TOP 1 atpub_codcall,atpub_codpob,atpub_codunivec,atpub_direccion,atpub_ncasa,atpub_cerro,atpub_sector,Telefono,Telefono_celular,mail FROM social_atenpub WITH (NOLOCK) WHERE atpub_rut='".$rutintefam."' AND atpub_codcall<>'' ORDER BY atpub_fechaat DESC";
			// echo $sql_mlb2."<br>";
			 $result2 = mssql_query($sql_mlb2, $lnk_social);
			// echo "SELECT TOP 1 * FROM social_atenpub WITH (NOLOCK) WHERE atpub_rut='".$rutintefam."' ORDER BY atpub_fechaat DESC"." aqui voy";
						 while($row222 = mssql_fetch_array($result2)) {
				$Rut=$rutfichafam;
				$descalle='';
				$uvecinal="";
				$nombrecompleto=$integrante_nombre;
				$codcalle= trim($row222["atpub_codcall"]);
				$codpob= trim($row222["atpub_codpob"]);
				$uvecinal=trim($row222["atpub_codunivec"]);
				$fono1=$row222["Telefono"];
				$fono2=$row222["Telefono_celular"];
				$email1=$row222["mail"];

				if ($codcalle <> "") {
				//$paso09="SELECT dc_Calle,dg_Calle FROM t_Calles WITH (NOLOCK) WHERE dc_Calle=".$codcalle;
				//$sql09 = mssql_query($paso09,$lnk_BDCAS);
				$paso09="SELECT COD_Calle, Descripcion FROM TT_CALLES WITH (NOLOCK) WHERE COD_Calle =".$codcalle;
					$sql09 = mssql_query($paso09,$lnk_obras);
					if (mssql_num_rows($sql09)>0){
						while($row09 = mssql_fetch_array($sql09)) {
						$descalle=trim($row09["Descripcion"]);
						} 
					}
				...
				$direccion=$descalle." ".trim($row222["atpub_direccion"])." ".trim($row222["atpub_ncasa"])."".$uvecinal." ".trim($row222["atpub_cerro"])." ".trim($row222["atpub_sector"]);
...
if (($haysocial != 1)&&($hayinspec == 1)){
	  $result = mssql_query("SELECT TOP 1 RutInfractor,NombreInfractor,DireccionInfractor,ComunaInfractor,TelefonoInfractor FROM Parte_M WITH (NOLOCK) WHERE RutInfractor='".$rut."' OR RutInfractor='".$rut1."' OR RutInfractor='".$rut2."' ORDER BY AnoParte DESC", $lnk_inspec) or die("MS-Query Error en la consulta a la BD");
	while($row = mssql_fetch_array($result)) {
...
if (($haysocial != 1)&&($hayaseo == 1)){
     // $result = mssql_query("SELECT TOP 1 Cont_Rol, Cont_Propietario, Cont_Direccion, Cont_Manzana, Cont_Predio, Cont_Avaluo, Cont_Rut, Cont_UVecinal, Cont_Exencion, Cont_Recargo, Cont_PuntCAS, Cont_Afecta, Cont_PorCAS, Cont_Exento, Cont_Sector, Cont_Descrip, Cont_Convenio, Cont_Telefono, Cont_Poblacion, Cont_FechaDecExen, Cont_NumeDecExen, Cont_Telefono2,  Cont_CobroDiferenciado, Cont_ClasificacionPredio, Cont_TipoRebaja, Cont_UsuarioRebaja, Cont_FechaRebaja, Email, DireccionPostal, CodigoComunaPostal, CodigoPostal, Construido, Propietario, ModificaRol_SII, FechaTerminoExencion FROM AsContribuyente WITH (NOLOCK) WHERE Cont_Rut='".$rut."' OR Cont_Rut='".$rut1."' OR Cont_Rut='".$rut2."' ORDER BY Cont_Rol DESC", $lnk_aseo) or die("MS-Query Error en la consulta a la BD");
	  $result = mssql_query("SELECT TOP 1 Cont_Rol, Cont_Propietario, Cont_Direccion, Cont_Manzana, Cont_Predio, Cont_Avaluo, Cont_Rut, Cont_UVecinal, Cont_Exencion, Cont_Recargo, Cont_PuntCAS, Cont_Afecta, Cont_PorCAS, Cont_Exento, Cont_Sector, Cont_Descrip,  Cont_Telefono, Cont_Poblacion, Cont_FechaDecExen, Cont_NumeDecExen, Cont_Telefono2,  Cont_CobroDiferenciado, Cont_ClasificacionPredio, Cont_TipoRebaja, Cont_UsuarioRebaja, Cont_FechaRebaja, Email, DireccionPostal, CodigoComunaPostal, CodigoPostal, Construido, Propietario, ModificaRol_SII, FechaTerminoExencion FROM AsContribuyente WITH (NOLOCK) WHERE Cont_Rut='".$rut."' OR Cont_Rut='".$rut1."' OR Cont_Rut='".$rut2."' ORDER BY Cont_Rol DESC", $lnk_aseo) or die("MS-Query Error en la consulta a la BD");
	while($row = mssql_fetch_array($result)) {
		$Rut=$row["Cont_Rut"];
...
if (($haysocial != 1)&&($haypatcom == 1)){
	  $result = mssql_query("SELECT TOP 1 RUT,NOMBRE,DIRECCION,COMUNA,FONO1,FONO2,CorreoElectronico,DireccionPostal FROM MAESTRO_CONTRIBUYENTE WITH (NOLOCK) WHERE RUT='".$rut."' OR RUT='".$rut1."' OR RUT='".$rut2."' ORDER BY ANO_Capital DESC", $lnk_patcom) or die("MS-Query Error en la consulta a la BD");
	while($row = mssql_fetch_array($result)) {
		$Rut=$row["RUT"];
...
<th>Fecha<br/>FPS</th><th>Motivo<br />Atenci&oacute;n</th>
<th>Resultado Atenci&oacute;n</th><th>Valor<br />Beneficio</th>
<th>Atendido por</th>
</tr></thead>
<?
  ...
  while($row2 = mssql_fetch_array($result2)) {
	  ...
	  $codasistent="";
	  if ($fecatencion <> "") {
	  $sql2 = mssql_query("SELECT TOP 5 atpub_codtpat,atpub_atendidopor FROM social_atenpub WITH (NOLOCK) WHERE atpub_rut='".$Rut."' and atpub_fechaat='".$fecatencion."' ORDER BY atpub_fechaat DESC",$lnk_social);
	  }
	 ...
	 $sql7 = mssql_query("SELECT Folio_atencion,Fecha_atencion,Codigo_Resultado,Codigo_Pertenece,Codigo_Asistente,Rut FROM Det_Resultado_Atencion WITH (NOLOCK) WHERE Rut LIKE '%".$Rut."%' AND Fecha_atencion='".$fecatencion."' ORDER BY Fecha_atencion DESC", $lnk_social);
	  if (mssql_num_rows($sql7)>0){
	    while($row5 = mssql_fetch_array($sql7)) {
				...
				if ($codresult != "") {
				$paso2="SELECT Descripcion_Resultado FROM Social_Resultado_Atencion WITH (NOLOCK) WHERE Codigo_Resultado ='". $codresult."'";
 ...
 <?if ($ver3==1) { ?>
  <!-- Patentes Comerciales -->    
                ...
                <div id="loginBoxPatCom">                
					...
					<input type="hidden" name="area" value="PatCom">
                        <fieldset class="cuerpo_gris" id="body">
		                  ...
		                  <!-- Anotaciones -->
			 <?
			 $result = mssql_query("SELECT TOP 10 RUT,FECHA_DECLARACION,TIPO_CAPITAL,MONTO_CAPITAL,MONTO_PORCENTAJE_CAPITAL,ANO_Capital FROM MAESTRO_CONTRIBUYENTE WITH (NOLOCK) WHERE RUT='".$rut."' OR RUT='".$rut1."' OR RUT='".$rut2."' ORDER BY ANO_Capital DESC", $lnk_patcom);
			 if (mssql_num_rows($result)>0){
			 ?>
              <table width='100%' border='1' cellspacing='0' cellpadding='0' class='bordered' >
	...
	while($row2 = mssql_fetch_array($sql2)) {
		...
		$sql4 = mssql_query($paso2, $lnk_obras);
		while($row4 = mssql_fetch_array($sql4)) {
                ...
                <div id="loginBoxLicCond">                
					...
					<input type="hidden" name="area" value="LicCond">
                        <fieldset class="cuerpo_gris" id="body">
		                  ...
		                  <!-- Anotaciones -->
			 <?
			 $result = mssql_query("SELECT TOP 10 Direccion,Comuna,Rut,Profesion,Fecha_Nacimiento FROM Personas WITH (NOLOCK) WHERE Rut='".$rut."' OR Rut='".$rut1."' OR Rut='".$rut2."'", $lnk_licconducir);
			 if (mssql_num_rows($result)>0){
			 ?>
              <table width='100%' border='1' cellspacing='0' cellpadding='0' class='bordered' >
		...
		$result = mssql_query("SELECT Cont_Rol, Cont_Propietario, Cont_Direccion, Cont_Avaluo, Cont_Rut, Cont_Exencion, Cont_Recargo, Cont_PuntCAS, Cont_Afecta, Cont_PorCAS, Cont_Exento, Cont_Sector, Cont_Descrip,  Cont_FechaDecExen, Cont_NumeDecExen, FechaTerminoExencion FROM AsContribuyente WITH (NOLOCK) WHERE Cont_Rut='".$rut."' OR Cont_Rut='".$rut1."' OR Cont_Rut='".$rut2."'", $lnk_aseo);
			 ...
			 <? 
				while($row = mssql_fetch_array($result)) {
					...
					$paso="SELECT TOP 18 Pag_FechaVen,Pag_Rol,Pag_FechaPago,Pag_Ano,Pag_Cuota,Pag_Valor,Pag_Total FROM AsPagos WITH (NOLOCK) WHERE Pag_Rol='". $numrol."' ORDER BY Pag_Ano DESC, Pag_FechaVen DESC";
					$sql3 = mssql_query($paso, $lnk_aseo);
					$n=0;
					if (mssql_num_rows($sql3)>0){
...
<!-- tesoreria -->    
	...
	?>
				 ...
				 <? $auxctcontrol=1;
					...
					while($row = mssql_fetch_array($resultB)) {
						...
						if ( strlen($cerglosa) == 0 ){
							$qryteso2="SELECT TOP 25 Ano_Proceso,Codigo_Area,NumeroCaja,Orden_Ingreso,IDOrden_Ingreso,Item_Pago,Correlativo_Ingreso,Monto,Devengar FROM DetalleIngresosDiarios WITH (NOLOCK) WHERE Item_Pago='".$itempago."' AND Orden_Ingreso='".$ordingreso."' AND Ano_Proceso='".$anhoproc."' ORDER BY Ano_Proceso DESC";
							$result9 = mssql_query($qryteso2, $lnk_teso);
							if (mssql_num_rows($result9)>0){
								while($row = mssql_fetch_array($result9)) { 
									 $correlingreso=trim($row["Correlativo_Ingreso"]);
 ...
 <dt><span class="tesoreria">Observaciones (WorkFlow)</span><? if ($haywf == 0) { echo "&nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; (<img src='images/warning_16.png' alt='No Hay Datos' border='0'/> No se encuentran datos en este sistema)&nbsp; &nbsp;"; }?></dt>
              ...
              <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabla_int">
      			<tr><td>
		 <?
	$pasoC="SELECT id_anotacion,observacion,fecha_creacion,area FROM anotaciones WHERE vigente=1 and rut_asociado='".$rut."' OR rut_asociado='".$rut1."' OR rut_asociado='".$rut2."' order by id_anotacion desc";
	...
	?>
	 ...
	 while($row_1=mysql_fetch_array($resultC)) {
			...
			$area=$row_1["area"];
	$pasobarea="SELECT desc_area FROM wf_areas WHERE nombre_area='".$area."'";
	$barea = mysql_query($pasobarea,$conexio);
  
	 while($row_barea=mysql_fetch_array($barea)) {
		$nombrearea=$row_barea["desc_area"];
	 }
	  $result_09 = "SELECT enviado_a,observacion,fecha_hora,rut_ciudadano FROM wf_workflow WHERE id_anotacion='".$id_anotacion."' order BY id_wf DESC";
