            ...
            <tr>
              <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabla_margin">
                <tr>
				  ...
				  <td width="106" align="left" class="Arial2">&nbsp;<input type="submit" class="rojo_sombra" value="&laquo; Volver" onclick="location='pidecalle.php'"/></td>
                  <td width="77" align="center">&nbsp;</td>
                  ...
                  <td width="269" align="center" valign="middle" class="tabla_cont_social"><input name="calle" type="text" class="campo_txt" id="calle" size="80" maxlength="100" onkeypress="return isAlpha2(event);"value=""  placeholder="Ej: Ecuador" style="width:320px !important;"/></td>
				  <td width="50" align="center" valign="middle" class="tabla_cont_social"><input name="numero" type="text" class="campo_txt" id="calle" size="50" maxlength="20" onkeypress="return isAlpha(event);" value=""  placeholder="Ej: 12345" style="width:100px !important;"/></td>
...
<?
	  ...
	  $result = mssql_query("SELECT * FROM MAESTRO_CONTRIBUYENTE WITH (NOLOCK) WHERE DIRECCION like '%".$_POST["calle"]."%".$_POST["numero"]."%'  order by DIRECCION asc ", $lnk_patcom) or die("MS-Query Error en la consulta a la BD");
...
<?
	...
	$result = mssql_query("SELECT Rut, Nombre, Domicilio, Comuna FROM Encabezado_Orden_Ingreso WITH (NOLOCK) WHERE Domicilio like '%".$_POST["calle"]."%".$_POST["numero"]."%'  order by Domicilio asc", $lnk_comun) or die("MS-Query Error en la consulta a la BD");
	while($row = mssql_fetch_array($result)) {
		$Rut=$row["Rut"];
...
<?
    ...
    $result = mssql_query("SELECT RUT, NOMBRES, DIRECCION, NUMERO, COMUNA  FROM Contribuyentes WITH (NOLOCK) WHERE  DIRECCION like '%".$_POST["calle"]."%' and NUMERO like '%".$_POST["numero"]."%'   order by DIRECCION asc, NUMERO asc", $lnk_provisorio) or die("MS-Query Error en la consulta a la BD");
	while($row = mssql_fetch_array($result)) {
		$Rut=$row["RUT"];
...
<?}?>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabla_int">
			...
			<th>Nombre</th><th>Rut</th><th>Direcci&oacute;n</th><th width="80">Ir al detalle</th></tr></thead>
<?
	  $result = mssql_query("SELECT  * FROM Parte_M WITH (NOLOCK) WHERE DireccionInfractor like '%".$_POST["calle"]."%".$_POST["numero"]."%'  order by DireccionInfractor asc", $lnk_inspec) or die("MS-Query Error en la consulta a la BD");
	while($row = mssql_fetch_array($result)) {
...
<?
      ...
      $result = mssql_query("SELECT Cont_Propietario, Cont_Direccion, Cont_Rut, Cont_Rol, Cont_Exencion, Cont_FechaDecExen FROM AsContribuyente WITH (NOLOCK) WHERE Cont_Direccion like '%".$_POST["calle"]."%".$_POST["numero"]."%' order by Cont_Direccion asc", $lnk_aseo) or die("MS-Query Error en la consulta a la BD");
	while($row = mssql_fetch_array($result)) {
		$Rut=$row["Cont_Rut"];
...
<?}?>
 ...
 <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabla_int">
			...
			<th>Nombre</th><th>Rut</th><th>Direcci&oacute;n</th><th width="80">Ir al detalle</th></tr></thead>
<?php
    $result = mssql_query("SELECT Rut, Nombre, Direccion, Comuna FROM Propietarios WITH (NOLOCK) WHERE direccion like '%".$_POST["calle"]."%".$_POST["numero"]."%' order by direccion asc", $lnk_permcirc) or die("MS-Query Error en la consulta a la BD");
	while($row = mssql_fetch_array($result)) {
		$Rut=$row["Rut"];
		$nombrecompleto=trim($row["Nombre"]);
...
<tr>
<td><?=$nombrecompleto?></td><td><?=$Rut?></td><td><?=$direccion?></td><td><input type="submit" class="rojo_sombra" value="&laquo; Ver Detalle"
onclick="window.open('mostrardatos.php?r=<?=$Rut?>&donde=1', 'Consulta Vecinos', 'width=1000,height=700,scrollbars=1,resizable=1'); return false;" /></td>
</tr>
<?}?>
</table>
	</td>
