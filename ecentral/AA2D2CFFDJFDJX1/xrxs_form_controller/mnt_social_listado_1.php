...
//Se validan si existe el usuario o el email asociado de este en la base de datos
	...
	if(isset($Nombre)){
		$sql_usuario = mysql_query("SELECT Nombre FROM mnt_social_listado WHERE Nombre='".$Nombre."'  "); 
...
//Validaciones de ingreso de datos obligatorios
	...
	//Valida el ingreso del id_socialcat
	if ( empty($id_socialcat) )      $errors[1] 	  = '
	<div id="txf_01" class="alert_error">  
	  	No ha seleccionado una categoria para el Evento Social
		<a class="closed_b" href="javascript:void(0);" onclick="document.getElementById(&apos;txf_01&apos;).className = &apos;oculto&apos;">
			<i class="fa fa-times"></i>
		</a>
	</div>';
	...
	<div id="txf_02" class="alert_error">  
	  	No ha ingresado un nombre para la atencion
	...
	//Valida el ingreso del Nombre
	if ( empty($Tipo) )      $errors[3] 	  = '
	<div id="txf_02" class="alert_error">  
	  	No ha seleccionado un tipo de atencion
		<a class="closed_b" href="javascript:void(0);" onclick="document.getElementById(&apos;txf_02&apos;).className = &apos;oculto&apos;">
			<i class="fa fa-times"></i>
		</a>
	</div>';
