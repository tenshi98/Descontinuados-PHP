...
$location.='&viewgrilla='.$_GET['viewgrilla'];
$location.='&tipo='.$_GET['tipo'];
$location.='&mod=true';$location.='&app_conf='.$_GET['app_conf'];
//Llamamos a las partes del formulario
require_once '../AA2D2CFFDJFDJX1/xrxs_form_common/app_areas_elementos.php';
...
$location.='?view='.$_GET['view'];
$location.='&viewgrilla='.$_GET['viewgrilla'];
$location.='&tipo='.$_GET['tipo'];$location.='&delete=true';
$location.='&app_conf='.$_GET['app_conf'];
//Llamamos al formulario de borrado
...
<div id="txf_03" class="alert_sucess">  Datos Creados correctamente
...
<div id="txf_03" class="alert_sucess">  Datos Modificados correctamente
<a class="closed_c" href="javascript:void(0);" onclick="document.getElementById(&apos;txf_03&apos;).className = &apos;oculto&apos;">	<i class="fa fa-times"></i></a>
</div>
<?php }?>
<?php  if (isset($_GET['delete'])) {?>
<div id="txf_03" class="alert_sucess">  Datos borrados correctamente
<a class="closed_c" href="javascript:void(0);" onclick="document.getElementById(&apos;txf_03&apos;).className = &apos;oculto&apos;">	<i class="fa fa-times"></i></a>
</div>
<?php }?>
<?php ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
...
$resultado = mysql_query ($query, $dbConn);
$rowdata = mysql_fetch_assoc ($resultado);	
//Se trae un listado con todos los margenes
$arrMargen = array();
$query = "SELECT  Nombre, Codigo
FROM `app_ajustes_tipo`
WHERE idGrupo=38
	...
	<div class="box dark">	
		...
		<div id="div-1" class="body">	
			<form class="form-horizontal" method="post">		
				...
				<div class="form-group">			
					<label for="text2" class="control-label col-lg-4">Tipo de margen</label>			
					<div class="col-lg-8">				
						<select class="form-control" tabindex="15" name="Margen" required>
							<option value="">Seleccione Tipo margen</option>
							<?php foreach ($arrMargen as $margen) { ?>
							<option value="<?php echo $margen['Codigo'] ?>" <?php if(isset($Margen)&&$Margen==$margen['Codigo']) {echo 'selected="selected"';}elseif($margen['Codigo']==$rowdata['Margen']){echo 'selected="selected"';}?>><?php echo $margen['Nombre'] ?></option>
							<?php } ?>       				
						</select>			
					</div>		
				</div>
				...
				<div class="form-group">			
					...
					<div class="col-lg-8">				
						<select class="form-control" tabindex="15" name="Margen" required>
							<option value="">Seleccione Tipo margen</option>
							<?php foreach ($arrMargen as $margen) { ?>
							<option value="<?php echo $margen['Codigo'] ?>" <?php if(isset($Margen)&&$Margen==$margen['Codigo']) {echo 'selected="selected"';}?>><?php echo $margen['Nombre'] ?></option>
							<?php } ?>       				
						</select>			
					</div>		
				</div>
...
<div class="form-group" style="margin-top:15px;">
<a href="<?php echo $location.'?new=true&app_conf='.$_GET['app_conf']; ?>" class="btn btn-default fright margin_width" >Crear Nueva Pantalla</a>
</div>
<div class="clearfix"></div>                       
