...
$original = "taxistas_vehiculos.php";
$location = $original;
...
if ( !empty($_POST['submit']) )  { 
	...
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_common/taxista_vehiculos.php';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_controller/taxista_vehiculos_1.php';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_insert/taxista_vehiculos.php';
}
...
if ( !empty($_POST['submit_edit']) )  { 
	...
	//Llamamos a las partes del formulario
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_common/taxista_vehiculos.php';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_controller/taxista_vehiculos_2.php';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_update/taxista_vehiculos.php';
}
//se borra un dato
if ( !empty($_GET['del']) )     {
	//Se agregan ubicaciones
	...
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_del/taxista_vehiculos.php';
}
...
function cambia_marca(){
	...
	document.form1.idModelo.options[0].selected = true
			...
			<div class="form-group">
           		<input type="hidden" value="<?php echo $_GET['view'] ?>" name="idPropietario">
...
$query = "SELECT 
taxista_vehiculos.PPU AS patente,
...
taxista_vehiculos.ftransferencia AS fecha_transferencia,
colors_1.Nombre AS color_base,
colors_2.Nombre AS color_complementario,
taxista_vehiculos.Ffabricacion AS fecha_fabricacion,
taxista_vehiculos.N_Motor AS numero_motor,
taxista_vehiculos.N_Chasis AS numero_chasis,
taxista_vehiculos.Obs AS observaciones
FROM `taxista_vehiculos`
LEFT JOIN `vehiculos_marcas`            ON vehiculos_marcas.idMarca              = taxista_vehiculos.idMarca
LEFT JOIN `vehiculos_modelos`           ON vehiculos_modelos.idModelo            = taxista_vehiculos.idModelo
LEFT JOIN `vehiculos_transmision`       ON vehiculos_transmision.idTransmision   = taxista_vehiculos.idTransmision
LEFT JOIN `vehiculos_colores`  colors_1 ON colors_1.idColorV                     = taxista_vehiculos.idColorV_1
LEFT JOIN `vehiculos_colores`  colors_2 ON colors_2.idColorV                     = taxista_vehiculos.idColorV_2
...
LEFT JOIN `vehiculos_colores`  colors_1 ON colors_1.idColorV                     = taxista_vehiculos.idColorV_1
LEFT JOIN `vehiculos_colores`  colors_2 ON colors_2.idColorV                     = taxista_vehiculos.idColorV_2
WHERE taxista_vehiculos.idPropietario={$_GET['view']}
...
?>
<div class="form-group">
<?php if ($rowlevel['level']>=3){?><a href="<?php echo $location.$x; ?>&new=true" class="btn btn-default fright margin_width" >Crear Nuevo Vehiculo</a><?php }?>
</div>
<div class="clearfix"></div> 
<div class="col-lg-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Vehiculos del Propietario</h5>
		</header>
    ...
    <?php foreach ($arrVehiculos as $vehiculos) { ?>
    	<tr class="odd">
			...
			<td class=" ">
<?php if ($rowlevel['level']>=1){?><a href="<?php echo $location.$x.'&view_vehiculo='.$vehiculos['idVehiculo']; ?>" title="Ver Informacion" class="icon-ver info-tooltip"></a><?php }?>
<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.$x.'&id='.$vehiculos['idVehiculo']; ?>" title="Editar Informacion" class="icon-editar info-tooltip"></a><?php }?>
<?php if ($rowlevel['level']>=4){?><?php $ubicacion=$location.$x.'&del='.$vehiculos['idVehiculo'];?>
...
$z="WHERE taxista_propietarios.idPropietario > 0";	
//Bloque de filtros
$x = '';
//Verifico si la variable de busqueda existe
if (isset($_GET['search']) && $_GET['search'] != ''){
	$z .=" AND taxista_propietarios.Rut LIKE '%{$_GET['search']}%'";
	$x .= "&search=".$_GET['search'] ; 	
}
if(isset($_GET['Nombre']) && $_GET['Nombre'] != '')  { 
	$z .= " AND taxista_propietarios.Nombre LIKE '%{$_GET['Nombre']}%' " ;
	$x .= "&Nombre=".$_GET['Nombre'] ; 
}
if(isset($_GET['Apellido_Paterno']) && $_GET['Apellido_Paterno'] != '')  { 
	$z .= " AND taxista_propietarios.Apellido_Paterno LIKE '%{$_GET['Apellido_Paterno']}%' " ;
	$x .= "&Apellido_Paterno=".$_GET['Apellido_Paterno'] ; 
}
if(isset($_GET['Apellido_Materno']) && $_GET['Apellido_Materno'] != '')  { 
	$z .= " AND taxista_propietarios.Apellido_Materno LIKE '%{$_GET['Apellido_Materno']}%' " ;
	$x .= "&Apellido_Materno=".$_GET['Apellido_Materno'] ; 
}
if(isset($_GET['rango_a']) && $_GET['rango_a'] != ''&&isset($_GET['rango_b']) && $_GET['rango_b'] != ''){ 
	$z .= " AND taxista_propietarios.fNacimiento BETWEEN '{$_GET['rango_a']}' AND '{$_GET['rango_b']}'" ;
	$x .= "&rango_a={$_GET['rango_a']}&rango_b={$_GET['rango_b']}" ;           
}
if(isset($_GET['Sexo']) && $_GET['Sexo'] != '')  { 
	$z .= " AND taxista_propietarios.Sexo = '".$_GET['Sexo']."'" ;
	$x .= "&Sexo=".$_GET['Sexo'] ; 
}
if(isset($_GET['idCiudad']) && $_GET['idCiudad'] != '')  { 
	$z .= " AND taxista_propietarios.idCiudad = '".$_GET['idCiudad']."'" ;
	$x .= "&idCiudad=".$_GET['idCiudad'] ; 
}
if(isset($_GET['idComuna']) && $_GET['idComuna'] != '')  { 
	$z .= " AND taxista_propietarios.idComuna = '".$_GET['idComuna']."'" ;
	$x .= "&idComuna=".$_GET['idComuna'] ; 
}

//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT idPropietario FROM `taxista_propietarios` ".$z."";
...
$arrUsers = array();
$query = "SELECT 
taxista_propietarios.idPropietario,
taxista_propietarios.Rut,
taxista_propietarios.Nombre,
taxista_propietarios.Apellido_Paterno,
taxista_propietarios.Apellido_Materno
FROM `taxista_propietarios`
".$z."
ORDER BY taxista_propietarios.Apellido_Paterno ASC
...
<div class="form-group">
<form class="form-horizontal" action="<?php echo $location ?>"  name="form1">
<label class="control-label col-lg-4">Buscar Propietarios</label>
    <div class="col-lg-5">
		<div class="input-group bootstrap-timepicker">
...
<div class="col-lg-12">
    ...
    <?php foreach ($arrUsers as $usuarios) { ?>
    	<tr class="odd">
			...
			<td class=" ">
<?php if ($rowlevel['level']>=1){?><a href="<?php echo $location.'&view='.$usuarios['idPropietario']; ?>" title="Ver Informacion" class="icon-ver info-tooltip"></a><?php }?>   
			</td>
		</tr>
    <?php } ?>                    
	</tbody>
