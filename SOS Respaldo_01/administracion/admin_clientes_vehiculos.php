...
// Se trae un listado con todas las comunas
 if ($rowdata['idMarca']!=0&&$rowdata['idMarca']!=''){$arrModelo = array();$query = "SELECT idModelo, idMarca, NombreFROM `vehiculos_modelos`WHERE idMarca = ".$rowdata['idMarca']." ORDER BY Nombre ASC ";$resultado = mysql_query ($query, $dbConn);while ( $row = mysql_fetch_assoc ($resultado)) {array_push( $arrModelo,$row );}
} 
...
document.form1.idModelo.options[i].value=id_modelo[i]	  
 ...
 }else{	document.form1.idModelo.length = 1	
...
document.form1.idModelo.options[0].selected = true
            ...
            <div class="form-group">			
				<label for="text2" class="control-label col-lg-4">Numero de Chasis</label>			
					<div class="col-lg-8">				<input type="text"  placeholder="Numero de Chasis" class="form-control"  name="N_Chasis" <?php if(isset($N_Chasis)) {echo 'value="'.$N_Chasis.'"';}?> required>			
				</div>		
			</div>
			...
			<div class="form-group">
           	<input type="hidden" value="<?php echo $_GET['view'] ?>" name="idCliente">			<input type="submit" id="text2"  class="btn btn-primary fright margin_width" value="Guardar Cambios" name="submit">
<?php //Se verifican las variables
$location = $original; 
$location .='?pagina='.$_GET['pagina'];
if(isset($_GET['search']) && $_GET['search'] != '')                                                     { $location .= "&search=".$_GET['search'] ; 	}
