...
foreach($row_tarea as $tipo=>$productos){ ?>
  <tr>
   ...
   if ($cat_anterior!=$productos[0]['cat']) {
	echo '<tr><td colspan="8" style="background-color: #EEE; font-size:18px; font-weight:bolder">'.$productos[0]['cat'].'</td></tr>';   
	 $cat_anterior=$productos[0]['cat'];
   } else {
	   $cat_anterior=$productos[0]['cat'];  
	   }
 if ($subcat_anterior!=$productos[0]['subcat']) {
	echo '<tr><td style="background-color: #EEE;" colspan="8">'.$productos[0]['subcat'].'</td></tr>';   
...
<tr> 
   <td style="background-color: #CCC;" colspan="7">Total del mes</td>
