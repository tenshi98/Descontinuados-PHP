...
$query = "select email from usuarios_listado where email='".$email."' AND mode='3'";
...
if(isset($email)){
	if($rowusr['email']!=$email){  
		$errors[2] 	  = '<h4 class="alert_error">El email ingresado no existe</h4>';
	}	
}
...
if ( !isset($errors[1]) && !isset($errors[2])   ) {
		...
		$query  = "UPDATE `usuarios_listado` SET password='".$nueva_clave."' WHERE email='".$email."' AND mode='3' ";
