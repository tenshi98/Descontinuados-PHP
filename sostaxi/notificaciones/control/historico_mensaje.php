...
<?
if ($_POST["dedonde"]=="menuppal") {
	$mensaje=$_POST["mensaje"];
}else{
	$mensaje=$_GET["mensaje"];
}
		$sql3="SELECT * FROM mensajes where  id=".$mensaje; 
		$gral3=mysql_query($sql3,$base);
		while($registro2=mysql_fetch_array($gral3)) 
		{ 
			$mensaje_texto=$registro2["mensaje"];
		}

	$sql2="SELECT * FROM visitas where id_mensaje=".$mensaje." order by fecha_hora desc LIMIT 0, 10"; 
...
{ 
	...
	map.addMarker({
        ...
        lng: <?=$lon?>,
        title: '<?=$fecha_hoy?>',
		//icon: '../images/board-AzulH_64.png',
	// LINK PARA VER EL DETALLE DE LA UBICACION
     infoWindow: {
          content: "<p><a href='http://<?=$nombreurl?>/control/detalle.php?lon=<?=$lon?>&lat=<?=$lat?>&fecha=<?=$fecha_hoy?>'>"+"Ver detalle</a>.</p>"
        }
	// LINK PARA VER EL DETALLE DE LA UBICACION
      });
...
<table align="center" width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <span class="google_fjalla_gde">Hist&oacute;rico por Mensajes(<?=$mensaje_texto?>)</span>	</td></tr>
</table>
