...
if ( !empty($_GET['id']) )     {
	...
	$form_obligatorios = '';
	$form_trabajo= 'del_robo';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/robos_listado.php';	
}
...
if (isset($_GET['deleted'])) {$error['usuario'] 	  = 'sucess/Robo silenciado correctamente';}
...
<script type="text/javascript">
	function actualiza(){
    	...
    	$("#consulta").load("robos_listado_consulta.php?typeuser=<?php echo $xx ?>");
		...
		if(jconsulta>0){ 
			$("#update").load("robos_listado_consulta_2.php?pagina=<?php echo $_GET["pagina"] ?>&typeuser=<?php echo $xx ?>");
		 } 
	}
