	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" name="form1">
				...
				echo form_select_filter('Conductor','idConductor', $x1, 2, 'idConductor', 'Nombre,Apellido_Paterno,Apellido_Materno', 'transantiago_conductores', 0, $dbConn);
				echo form_select_filter('Propietario','idPropietario', $x2, 2, 'idPropietario', 'Nombre,Apellido_Paterno,Apellido_Materno', 'transantiago_propietarios', 0, $dbConn);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" name="form1">
				...
				if(isset($idConductor)) {    $x4  = $idConductor;    }else{$x4  = '';}
				if(isset($idPropietario)) {  $x5  = $idPropietario;  }else{$x5  = '';}
				if(isset($idRecorrido)) {    $x6  = $idRecorrido;    }else{$x6  = '';}
				if(isset($PPU)) {            $x7  = $PPU;            }else{$x7  = '';}
				if(isset($idMarca)) {        $x8  = $idMarca;        }else{$x8  = '';}
				if(isset($idModelo)) {       $x9  = $idModelo;       }else{$x9  = '';}
				if(isset($idTransmision)) {  $x10  = $idTransmision; }else{$x10 = '';}
				if(isset($idColorV_1)) {     $x11  = $idColorV_1;    }else{$x11 = '';}
				if(isset($idColorV_2)) {     $x12  = $idColorV_2;    }else{$x12 = '';}
				if(isset($fTransferencia)) { $x13 = $fTransferencia; }else{$x13 = '';}
				if(isset($fFabricacion)) {   $x14 = $fFabricacion;   }else{$x14 = '';}
				if(isset($N_Motor)) {        $x15 = $N_Motor;        }else{$x15 = '';}
				if(isset($N_Chasis)) {       $x16 = $N_Chasis;       }else{$x16 = '';}
				if(isset($Obs)) {            $x17 = $Obs;            }else{$x17 = '';}
				if(isset($idTipoCliente)) {  $x18 = $idTipoCliente;  }else{$x18 = '';}
				...
				echo form_select_filter('Conductor','idConductor', $x4, 2, 'idConductor', 'Nombre,Apellido_Paterno,Apellido_Materno', 'transantiago_conductores', 0, $dbConn);
				echo form_select_filter('Propietario','idPropietario', $x5, 2, 'idPropietario', 'Nombre,Apellido_Paterno,Apellido_Materno', 'transantiago_propietarios', 0, $dbConn);
				echo form_select_filter('Recorrido','idRecorrido', $x6, 2, 'idRecorrido', 'Nombre', 'transantiago_recorridos', 0, $dbConn);
				echo form_input('text', 'Patente', 'PPU', $x7, 2);
				echo form_select_depend1('Marca','idMarca', $x8, 2, 'idMarca', 'Nombre', 'vehiculos_marcas', 0,
										'Modelo','idModelo', $x9, 2, 'idModelo', 'idMarca', 'Nombre', 'vehiculos_modelos', 0, 
										 $dbConn);
				echo form_select('Tipo de Transmision','idTransmision', $x10, 2, 'idTransmision', 'Nombre', 'vehiculos_transmision', 0, $dbConn);						 
				echo form_select('Color Base','idColorV_1', $x11, 2, 'idColorV', 'Nombre', 'vehiculos_colores', 0, $dbConn);						 
				echo form_select('Color Complementario','idColorV_2', $x12, 1, 'idColorV', 'Nombre', 'vehiculos_colores', 0, $dbConn);
				echo form_date('Fecha de Transferencia','fTransferencia', $x13, 1);
				echo form_date('Fecha de Fabricacion','fFabricacion', $x14, 2);
				echo form_input('text', 'Numero de Motor', 'N_Motor', $x15, 2);
				echo form_input('text', 'Numero de Chasis', 'N_Chasis', $x16, 2);
				echo form_obs_simple('Observaciones', 'Obs', $x17, 1);
				if($arrUsuario['tipo']=='SuperAdmin'){
				echo form_select('Sistema','idTipoCliente', $x18, 2, 'idTipoCliente', 'Nombre', 'clientes_tipos', 0, $dbConn);
				}else{
				echo '<input type="hidden" name="idTipoCliente"   value="'.$arrUsuario['idTipoCliente'].' ">';
				}
				?>   
				...
				<div class="form-group">
					...
					<input type="hidden" name="idDisponibilidad" value="2" >
...
<div class="col-lg-12">
    ...
    <?php foreach ($arrUsers as $usuarios) { ?>
    	<tr class="odd">
			...
			<td>
<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_tran_bus.php?view='.$usuarios['idCliente']; ?>" title="Ver Informacion" class="icon-ver info-tooltip"></a><?php }?>   
