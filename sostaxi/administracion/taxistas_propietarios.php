...
if ( !empty($_POST['submit']) )  { 
	...
	$form_obligatorios = 'Nombre,Apellido_Paterno,Apellido_Materno,Fono1,email,Rut,NombreEmpresa';
	$form_trabajo= 'insert';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/taxista_propietarios.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$form_obligatorios = 'idPropietario,Nombre,Apellido_Paterno,Apellido_Materno,Fono1,email,Rut,NombreEmpresa';
	$form_trabajo= 'update';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/taxista_propietarios.php';
}
//se borra un dato
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			echo form_input('text', 'Telefono 1', 'Fono1', $x5, 2);
			echo form_input('text', 'Telefono 2', 'Fono2', $x6, 1);
			echo form_input('text', 'Email', 'email', $x7, 2);
			echo form_input('text', 'Rut', 'Rut', $x8, 2);
			echo form_date('F Nacimiento','fNacimiento', $x9, 1);
 ...
 <div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" action="<?php echo $location ?>" name="form1" >
			...
			if(isset($Empresa)) {            $x9  = $Empresa;           }else{$x9  = '';}
			
			//se dibujan los inputs
			echo form_input('text', 'Nombre', 'Nombre', $x1, 1);
			echo form_input('text', 'Apellido Paterno', 'Apellido_Paterno', $x2, 1);
			echo form_input('text', 'Apellido Materno', 'Apellido_Materno', $x3, 1);
			echo form_date('Fecha Nacimiento - Inicio','rango_a', $x4, 1);
			echo form_date('Fecha Nacimiento - Termino','rango_b', $x5, 1);
			echo form_select('Sexo','Sexo', $x6, 1, 'Inicial', 'Nombre', 'clientes_sexo', 0, $dbConn);
			echo form_select_depend1('Ciudad','idCiudad', $x7, 1, 'idCiudad', 'Nombre', 'mnt_ubicacion_ciudad', 0,
									'Comuna','idComuna', $x8, 1, 'idComuna', 'idCiudad', 'Nombre', 'mnt_ubicacion_comunas', 0, 
									 $dbConn);
			echo form_input('text', 'Nombre Empresa', 'Empresa', $x9, 1);
			?>
...
if (isset($_GET['search']) && $_GET['search'] != ''){        $z .=" AND taxista_propietarios.Rut LIKE '%{$_GET['search']}%'";	}
if(isset($_GET['Nombre']) && $_GET['Nombre'] != '')  {       $z .= " AND taxista_propietarios.Nombre LIKE '%{$_GET['Nombre']}%' " ;}
if(isset($_GET['Sexo']) && $_GET['Sexo'] != '')  {           $z .= " AND taxista_propietarios.Sexo = '".$_GET['Sexo']."'" ;}
if(isset($_GET['idCiudad']) && $_GET['idCiudad'] != '')  {   $z .= " AND taxista_propietarios.idCiudad = '".$_GET['idCiudad']."'" ;}
if(isset($_GET['idComuna']) && $_GET['idComuna'] != '')  {   $z .= " AND taxista_propietarios.idComuna = '".$_GET['idComuna']."'" ;}
if(isset($_GET['Empresa']) && $_GET['Empresa'] != '')  {     $z .= " AND taxista_propietarios.NombreEmpresa LIKE '%{$_GET['Empresa']}%' " ;}
