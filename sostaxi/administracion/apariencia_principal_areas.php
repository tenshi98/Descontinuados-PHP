...
$query = "SELECT  Nombre, grid_color, grid_border, grid_shadow, body_col, body_fil, url_img, grid_img, cat_img
		...
		<div id="div-1" class="body">	
			<form class="form-horizontal" method="post" name="form1">
				...
				<?php if ($_GET['tipo']=='tb_1-2'or$_GET['tipo']=='tb_1-3'or$_GET['tipo']=='tb_1-4'or$_GET['tipo']=='tb_1-5' ){ 
					...
					if(isset($cat_img)) {       $x7  = $cat_img;       }else{$x7  = $rowdata['cat_img'];}
					if(isset($url_img)) {       $x8  = $url_img;       }else{$x8  = $rowdata['url_img'];}
					if(isset($grid_img)) {      $x9  = $grid_img;      }else{$x9  = $rowdata['grid_img'];}
					...
					echo form_select_depend1('Categoria','cat_img', $x7, 2, 'idCatimg', 'Nombre', 'mnt_imagenes_categorias', 0,
										'Imagen','url_img', $x8, 2, 'file', 'idCatimg', 'Nombre', 'mnt_imagenes_listado', 0, 
										$dbConn);
					echo form_select('Ancho de la imagen','grid_img', $x9, 2, 'Codigo', 'Nombre', 'app_ajustes_tipo', 'idGrupo=24', $dbConn);
				
				}elseif($_GET['tipo']=='tb_img'){ 
					//Se verifican si existen los datos
					if(isset($Nombre)) {        $x1  = $Nombre;        }else{$x1  = $rowdata['Nombre'];}
					if(isset($cat_img)) {       $x2  = $cat_img;       }else{$x2  = $rowdata['cat_img'];}
					if(isset($url_img)) {       $x3  = $url_img;       }else{$x3  = $rowdata['url_img'];}
					
					//se dibujan los inputs
					echo form_input('text', 'Nombre de la imagen', 'Nombre', $x1, 2);
					echo form_select_depend1('Categoria','cat_img', $x2, 2, 'idCatimg', 'Nombre', 'mnt_imagenes_categorias', 0,
										'Imagen','url_img', $x3, 2, 'file', 'idCatimg', 'Nombre', 'mnt_imagenes_listado', 0, 
										$dbConn);
		 ...
		 <div id="div-1" class="body">	
			<form class="form-horizontal" method="post" name="form1">
				...
				<?php if ($_GET['tipo']=='tb_1-2'or$_GET['tipo']=='tb_1-3'or$_GET['tipo']=='tb_1-4'or$_GET['tipo']=='tb_1-5' ){ 
					...
					if(isset($cat_img)) {       $x7  = $cat_img;       }else{$x7  = '';}
					if(isset($url_img)) {       $x8  = $url_img;       }else{$x8  = '';}
					if(isset($grid_img)) {      $x9  = $grid_img;      }else{$x9  = '';}
					...
					//Se verifican si existen los datos
					if(isset($Nombre)) {        $x1  = $Nombre;        }else{$x1  = '';}
					if(isset($cat_img)) {       $x2  = $cat_img;       }else{$x2  = '';}
					if(isset($url_img)) {       $x3  = $url_img;       }else{$x3  = '';}
...
<div class="box">	
	<header>		
		<div class="icons"><i class="fa fa-table"></i></div><h5>Modificacion de los elementos de la grilla de <?php echo $_GET['nombre']?></h5>	
	</header>
...
<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
    ...
    <?php $grill = 1;foreach ($arrGrilla as $grilla) { ?>
    	<tr class="odd">		
			...
			<td>
			<a href="<?php echo $location.'&viewgrilla='.$grilla['idArea'].'&tipo='.$grilla['Tipo'].'&nombre='.$grilla['Nombre']; ?>" title="Ver Informacion" class="icon-ver info-tooltip"></a> 
