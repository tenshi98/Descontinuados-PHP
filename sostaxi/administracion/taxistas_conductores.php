...
$original = "taxistas_conductores.php";
$location = $original;
...
if ( !empty($_POST['submit']) )  { 
	...
	$form_obligatorios = 'Nombre,Apellido_Paterno,Apellido_Materno,Fono1,email,Rut';
	$form_trabajo= 'insert';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/taxista_conductores.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$form_obligatorios = 'idConductor,Nombre,Apellido_Paterno,Apellido_Materno,Fono1,email,Rut';
	$form_trabajo= 'update';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/taxista_conductores.php';
}
//se borra un dato
...
if (isset($_GET['created'])) {$error['usuario'] 	  = 'sucess/Conductor creado correctamente';}
if (isset($_GET['edited']))  {$error['usuario'] 	  = 'sucess/Conductor editado correctamente';}
if (isset($_GET['deleted'])) {$error['usuario'] 	  = 'sucess/Conductor borrado correctamente';}
	...
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			<div class="form-group">
            	<input type="hidden" name="idConductor"  value="<?php echo $_GET['id']; ?>" >
...
$z="WHERE taxista_conductores.idConductor > 0";	
//Verifico si la variable de busqueda existe
if (isset($_GET['search']) && $_GET['search'] != ''){        $z .=" AND taxista_conductores.Rut LIKE '%{$_GET['search']}%'";}
if(isset($_GET['Nombre']) && $_GET['Nombre'] != '')  {       $z .= " AND taxista_conductores.Nombre LIKE '%{$_GET['Nombre']}%' " ;}
if(isset($_GET['Sexo']) && $_GET['Sexo'] != '')  {           $z .= " AND taxista_conductores.Sexo = '".$_GET['Sexo']."'" ; }
if(isset($_GET['idCiudad']) && $_GET['idCiudad'] != '')  {   $z .= " AND taxista_conductores.idCiudad = '".$_GET['idCiudad']."'" ; }
if(isset($_GET['idComuna']) && $_GET['idComuna'] != '')  {   $z .= " AND taxista_conductores.idComuna = '".$_GET['idComuna']."'" ;}
if(isset($_GET['Apellido_Paterno']) && $_GET['Apellido_Paterno'] != '')  { 
	$z .= " AND taxista_conductores.Apellido_Paterno LIKE '%{$_GET['Apellido_Paterno']}%' " ;
}
if(isset($_GET['Apellido_Materno']) && $_GET['Apellido_Materno'] != '')  { 
	$z .= " AND taxista_conductores.Apellido_Materno LIKE '%{$_GET['Apellido_Materno']}%' " ;
}
if(isset($_GET['rango_a']) && $_GET['rango_a'] != ''&&isset($_GET['rango_b']) && $_GET['rango_b'] != ''){ 
	$z .= " AND taxista_conductores.fNacimiento BETWEEN '{$_GET['rango_a']}' AND '{$_GET['rango_b']}'" ;           
}

//Realizo una consulta para saber el total de elementos existentes
$query1 = "SELECT idConductor FROM `taxista_conductores` ".$z."";
...
$arrUsers = array();
$query = "SELECT 
taxista_conductores.idConductor,
...
FROM `taxista_conductores`
".$z."
ORDER BY taxista_conductores.Nombre ASC
...
<div class="col-lg-12">
    ...
    <?php foreach ($arrUsers as $usuarios) { ?>
    	<tr class="odd">
			...
			<td>
			<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_tax_conductor.php?view='.$usuarios['idConductor']; ?>" title="Ver Informacion" class="icon-ver info-tooltip"></a><?php }?>   
