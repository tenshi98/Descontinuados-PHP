...
if ( !empty($_POST['submit']) )  {
	...
	if ( !empty($_POST['linean']) )				$linea                = $_POST['linean'];
	...
	if ( empty($region) )       $errors[16] 			= '<div class="alert-error" >No ha ingresado Region para la Linea</div>';	
	if ( empty($comuna) )       $errors[17] 			= '<div class="alert-error" >No ha ingresado su Comuna para la Linea</div>';	
	if ( empty($linea) )       $errors[17] 				= '<div class="alert-error" >No ha ingresado Linea de Colectivos</div>';	
	
	

	
	if ( !isset($errors[1]) && !isset($errors[2]) && !isset($errors[3]) && !isset($errors[4]) && !isset($errors[5]) && !isset($errors[6]) && !isset($errors[7]) && !isset($errors[8]) && !isset($errors[9]) && !isset($errors[10]) && !isset($errors[11]) && !isset($errors[12]) && !isset($errors[13]) && !isset($errors[14]) && !isset($errors[15]) && !isset($errors[16]) && !isset($errors[17]) && !isset($errors[18])) {
...
$juntos=$uno.$dos;			
		...
		$query  = "INSERT INTO `ejecutivos` (nom_ejecutiv, fono_ejecutiv, ciudad_ejecutiv, login,pass, fecha_ingreso, mail_ejecutiv, grupo, radio, publicidad, estado_usr, codigo_sms,gcmcode,imei,lon,lat,dir_ejecutiv,rut,region,comuna_linea,linea) VALUES ('{$nom_ejecutiv}','{$telefono}','{$ciudad_ejecutiv}','{$login}','1234',{$fecha_ing},'{$login}','{$grupo}', {$radio}, '{$publicidad}', '{$estado_usr}', '{$codigo_sms}', '{$gcmcode}', '{$imei}', {$longitud}, {$latitud}, '{$dir_ejecutiv}','{$juntos}','{$region}','{$comuna}','{$linea}'  )";
		$result = mysql_query($query, $dbConn);
		

$sql ="update padron_electoral_CL set domicilio_2='{$dir_ejecutiv}', comuna_2='{$ciudad_ejecutiv}', tipo_domicilio2='RES', correo='{$login}',fono_celular='{$fono_ejecutiv}'  WHERE rut_compara='" . $juntos . "'";
...
$fecha=$diadis."/".$mesdis."/".$Anio."  ".$hora_muestra;
		...
		//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		//====== DE QUIEN ES ========
		$mail->From="informacion@sostaxi.cl";
		$mail->FromName="sostaxi";
		$mail->Sender="informacion@sostaxi.cl";
		$mail->AddReplyTo("informacion@sostaxi.cl", "Responde a este mail");
		//====== PARA QUIEN =========
...
$strBody = $strBody . "<td ><b>".$nom_ejecutiv."</b>, te damos la bienvenida a <b>sostaxi</b> y te recordamos que para el uso de nuestro sistema, debes utilizar el menu de tu dispositivo, ah&iacute; encontrar&aacute;s los siguientes Item:</td>";
...
$strBody =  $strBody . "<tr><td ><p>Tu cuenta ya ha sido creada,sin embargo debes activarla con el codigo enviado a traves de un SMS<br/> Tu codigo de activacion es : <b>".$codigo_sms."</b></p></td></tr></table>"; 
	...
	if ( !empty($_POST['nom_ejecutiv']) )       $nom_ejecutiv         = $_POST['nom_ejecutiv'];
	if ( !empty($_POST['fono_ejecutiv']) )      $fono_ejecutiv        = $_POST['fono_ejecutiv'];
	if ( !empty($_POST['comuna']) )				$ciudad_ejecutiv      = $_POST['comuna'];
	if ( !empty($_POST['dir_ejecutiv']) )		$dir_ejecutiv         = $_POST['dir_ejecutiv'];
	if ( !empty($_POST['login']) )              $login                = $_POST['login'];
}
/**********************************************************************************************************************************/
...
<tr><td><label>Comuna (Residencia)</label></td></tr>
<tr>
<td>
     <select name="ciudad_ejecutiv" required="required" class="ciudad_ejecutiv">
		<option value='<?php  if (isset($ciudad_ejecutiv)) {echo $ciudad_ejecutiv;}?>'><?php  if (isset($ciudad_ejecutiv)) {echo $ciudad_ejecutiv;}else{echo "Elija una Comuna";}?></option>
		<?php //consulta
...
<table class="table_msg">
<tr><td>Seleccione (ORIGEN):<br></td></tr>
<tr>
<td>
