...
if ($resultado) {
	...
	foreach ($arrRobos as $noti) {
		echo '<li>';
			$texto  = '';
			$texto .= $noti['marca'].' '.$noti['modelo'].' patente '.$noti['Patente'].' color '.$noti['color_base'];
			if(isset($noti['color_comp'])&&$noti['color_comp']!=''){ $texto .= ' con color '.$noti['color_comp'];}
			echo noti_title('',$texto,$config_app);
			echo noti_text('p','<strong>'.Fecha_completa($noti['Fecha']).'</strong>',$config_app);			
			echo '<table>';
				echo '<tr height="32px">';
					echo '<td width="70%"></td>'; 
					echo '<td width="30%">';
						echo table_btn('acept','fa-eye','notificaciones.php'.$w.'&view_robo='.$noti['idRobo'],'Ver','',$config_app); 
					echo '</td>';
				echo '</tr>';
			echo '</table>';
		echo '</li>';		
