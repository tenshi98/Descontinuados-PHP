...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$sql_nombre = mysql_query("SELECT Nombre FROM app_grilla WHERE Nombre='".$Nombre."'  "); 
				$n1 = mysql_num_rows($sql_nombre);
			} else {$n1=0;}
			if($n1 > 0) {$error['Nombre'] = 'error/El nombre de la grilla ya existe';}
			
			//Se verifica si el dato existe
			if(isset($Codigo)){
				$sql_codigo = mysql_query("SELECT Codigo FROM app_grilla WHERE Codigo='".$Codigo."'  ");
				$n1 = mysql_num_rows($sql_codigo);
			} else {$n1=0;}
			if($n1 > 0) {$error['Codigo'] = 'error/El codigo de la grilla ya existe';}
