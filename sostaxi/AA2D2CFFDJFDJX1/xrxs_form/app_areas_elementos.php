...
/*******************************************************************************************************************/
	...
	if ( !empty($_POST['cat_img']) )        $cat_img 	    = $_POST['cat_img'];
	...
	if ( !empty($_POST['idTipoContacto']) ) $idTipoContacto = $_POST['idTipoContacto'];
...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			...
			case 'cat_img':        if(empty($cat_img)){         $error['cat_img']        = 'error/No ha ingresado la carpeta de mp3';}break;
			...
			case 'idTipoContacto': if(empty($idTipoContacto)){  $error['idTipoContacto'] = 'error/No ha ingresado la web de talento';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if ( empty($error) ) {
				...
				if(isset($cat_img) && $cat_img != ''){               $a .= ",'".$cat_img."'" ;         }else{$a .= ",''";}
				...
				if(isset($idTipoContacto) && $idTipoContacto != ''){ $a .= ",'".$idTipoContacto."'" ;  }else{$a .= ",''";}
				...
				grid_border, grid_shadow, grid_img, cat_img, url_img, idTipoBoton, Archivo, idTipoAlerta, cercanos, contactar,idTipoContacto, desplegar , login, 
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				if(isset($cat_img) && $cat_img != ''){                 $a .= ",cat_img='".$cat_img."'" ;}
				...
				if(isset($idTipoContacto) && $idTipoContacto != ''){   $a .= ",idTipoContacto='".$idTipoContacto."'" ;}
...
/*******************************************************************************************************************/
		case 'down_elemento':	
			...
			$idElementos  = $_GET['down_elemento'];
			$Orden        = $_GET['orden'];
			$busqueda     = $Orden+1;
			//Primero busco el item que este sobre para actualizarle el orden
			$query = "SELECT  idElementos FROM `app_areas_elementos` WHERE Orden = {$busqueda} AND idArea = {$_GET['viewgrilla']}";
