...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			case 'idConductor':      if(empty($idConductor)){      $error['idConductor']       = 'error/No ha ingresado el id del sistema';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)&&isset($Apellido_Paterno)&&isset($Apellido_Materno)){
				$sql_usuario = mysql_query("SELECT Nombre FROM transantiago_conductores WHERE Nombre='".$Nombre."' AND Apellido_Paterno='".$Apellido_Paterno."' AND Apellido_Materno='".$Apellido_Materno."' "); 
			...
			if(isset($Rut)){
				$sql_email = mysql_query("SELECT Rut FROM transantiago_conductores WHERE Rut='".$Rut."'  ");
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `transantiago_conductores` (Nombre, Apellido_Paterno, Apellido_Materno, Rut, Sexo, fNacimiento, email, Pais, idCiudad, idComuna, Direccion, Fono1, Fono2) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `transantiago_conductores` SET ".$a." WHERE idConductor = '$idConductor'";
				$result = mysql_query($query, $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `transantiago_conductores` WHERE idConductor = {$_GET['del']}";
			$result = mysql_query($query, $dbConn);	
