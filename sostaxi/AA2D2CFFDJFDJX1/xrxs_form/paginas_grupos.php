...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			case 'idPagGrupo':     if(empty($idPagGrupo)){     $error['idPagGrupo']     = 'error/No ha ingresado el id del sistema';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$sql_usuario = mysql_query("SELECT Nombre FROM paginas_grupos WHERE Nombre='".$Nombre."' AND idTipoCliente='".$idTipoCliente."' "); 
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `paginas_grupos` (idTipoCliente, Nombre) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `paginas_grupos` SET ".$a." WHERE idPagGrupo = '$idPagGrupo'";
				$result = mysql_query($query, $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `paginas_grupos` WHERE idPagGrupo = {$_GET['del']}";
			$result = mysql_query($query, $dbConn);	
