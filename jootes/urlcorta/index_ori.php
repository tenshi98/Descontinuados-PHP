...
list($tipo,$video) = split('-',$_GET["video"]);

	$sql_videosdh="SELECT * FROM dh_videos_correos where id_orden=".$video;
	...
	{ 
		...
		$visitas=$visitas+1;
		$sql_visitas="update dh_videos_correos set visitas=". $visitas ." where id_orden=".$video;
...
<script src="http://<?=$nombreurl?>/jwp/js/jwplayer.js"></script>
<?if ($tipo=="DH") {
	$infor="http://".$nombreurl."/videos/".$nombre_video.".flv";
	?>
			...
			<script type="text/javascript"> 
				...
				skin: "http://<?=$nombreurl?>/jw/glow.zip",
								width: 692,
								height: 390,
		plugins: "http://<?=$nombreurl?>/jwp/js/adawe.swf",
		"adawe.type": "link",
		...
		modes: [
			{ type: "flash", src: "http://<?=$nombreurl?>/jwp/js/player.swf" },
			{ type: "html5" }
		]
				}); 
			</script> 
