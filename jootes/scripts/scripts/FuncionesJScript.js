...
function JDigitoRut(numero){  //al ingresar el valor del RUT
  var suma=0;				  //devuelve el DV
  var mul=2;
  var i=0;
...
function JOnFocoFechaInivip(Obj){ //Mes Año
	var a=0;
	a = Obj.value;
	if (a.length == 7){
		aux = Obj.value;
		Obj.value = aux.substring(0,2) + aux.substring(3,7) ;
	}
	Obj.select();
}

function JOnDejaFechaInivip(Obj){
	var EsError,a;
	if (Obj.value == ""){
		return;}
	aux = Obj.value;
	...
	EsError=CheckDateInivip(Obj.value)
	if (EsError == "si"){
		alert ("Fecha Inválida");
		if (!document.all){
			Obj.value = "";
		...
		Obj.value = aux.substring(0,2) + "/" + aux.substring(2,6);
	}
}

function CheckDateInivip(THISDATE) {  //esta funcion chequea si una fecha Inivip es valida, 
	var err="no";          // formatos mmaaaa  mm/aaaa mm-aaaa
	err = "no";
	a=THISDATE;	
	if (a.length == 7){
		d = a.substring(0, 2);   // mes
		c = a.substring(2, 3);   // '/'
		b = a.substring(3, 7);   // ano
	}
	else{
		d = a.substring(0, 2);   // mes
		c = "";   // '/'
		b = a.substring(2, 6);   // ano
	}
...
function EsPatente(Patente)
{
	if ((Patente.length=6)||(Patente.length=0))
	{
		...
		{
			var texto = Patente.substr(0,4);
			var num = Patente.substr(4,2);
			if ((EsNumero(num))&&(EsTexto(texto)))
			{
				return true;
			}
