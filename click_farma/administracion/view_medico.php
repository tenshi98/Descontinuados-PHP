...
$query = "SELECT  
medicos_listado.email, 
medicos_listado.Nombre, 
medicos_listado.Rut, 
medicos_listado.fNacimiento, 
medicos_listado.Direccion, 
medicos_listado.Fono1, 
medicos_listado.Especialidad,
...
medicos_disponibilidad.Nombre AS Disponibilidad,
medicos_sexo.Nombre AS Sexo


FROM `medicos_listado`
LEFT JOIN `ubicacion_departamento`   ON ubicacion_departamento.idDepartamento     = medicos_listado.idDepartamento
LEFT JOIN `ubicacion_provincia`      ON ubicacion_provincia.idProvincia           = medicos_listado.idProvincia
LEFT JOIN `ubicacion_distrito`       ON ubicacion_distrito.idDistrito             = medicos_listado.idDistrito
LEFT JOIN `medicos_disponibilidad`   ON medicos_disponibilidad.idDisponibilidad   = medicos_listado.idDisponibilidad
LEFT JOIN `medicos_sexo`             ON medicos_sexo.idSexo                       = medicos_listado.idSexo
WHERE medicos_listado.idMedico = {$_GET['view']}";
...
LEFT JOIN `usuarios_listado`   ON usuarios_listado.idUsuario     = medicos_observaciones.idUsuario
...
$arrLlamadas = array();
$query = "SELECT 
medicos_llamadas.Fecha, 
medicos_llamadas.Hora,
clientes_listado.Nombre AS Cliente
FROM `medicos_llamadas`
LEFT JOIN `clientes_listado`   ON clientes_listado.idCliente     = medicos_llamadas.idCliente
WHERE medicos_llamadas.idMedico = {$_GET['view']}
ORDER BY medicos_llamadas.Fecha DESC , medicos_llamadas.Hora DESC
LIMIT 25 ";
$resultado = mysql_query ($query, $dbConn);
while ( $row = mysql_fetch_assoc ($resultado)) {
	...
	<div class="box">
		<header>
			...
			<ul class="nav nav-tabs pull-right">
				...
				<li class=""><a href="#llamadas" data-toggle="tab">Llamadas</a></li>
        ...
        <div id="div-3" class="tab-content">
			...
			<div class="tab-pane fade active in" id="basicos">
				<div class="wmd-panel">
					...
					<p class="text-muted"><strong>DNI : </strong><?php echo $rowdata['Rut']; ?></p>
					...
					<p class="text-muted"><strong>Especialidad : </strong><?php echo $rowdata['Especialidad']; ?></p>
					<p class="text-muted"><strong>Disponibilidad : </strong><?php echo $rowdata['Disponibilidad']; ?></p>
					
					<h2 class="text-primary">Datos de Contacto</h2>
					<p class="text-muted"><strong>Email : </strong><?php echo $rowdata['email']; ?></p>
					<p class="text-muted"><strong>Telefono : </strong><?php echo $rowdata['Fono1']; ?></p>
				...
				<div class="wmd-panel">
					...
					<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
						...
						<?php foreach ($arrLlamadas as $llamadas) { ?>
							<tr class="odd">		
								<td><?php echo $llamadas['Cliente']; ?></td>
