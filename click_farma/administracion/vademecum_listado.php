...
$original = "vademecum_listado.php";
$location = $original;
...
if ( !empty($_POST['submit']) )  { 
	...
	$form_trabajo= 'insert';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/vademecum_listado.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$form_obligatorios = 'idVademecum,idCategoria,Titulo,Descripcion';
	$form_trabajo= 'update';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/vademecum_listado.php';
}
//se borra un dato
...
FROM `vademecum_listado`
WHERE idVademecum = {$_GET['id']}";
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			echo form_select('Categoria','idCategoria', $x1, 2, 'idCategoria', 'Nombre', 'vademecum_categoria', 0, $dbConn);
			echo form_input('text', 'Titulo', 'Titulo', $x2, 2); 
			echo form_textarea('Descripcion', 'Descripcion', $x3, 2);
			?>
        
			<div class="form-group">
            	<input type="hidden" name="idVademecum" value="<?php echo $_GET['id']; ?>" >
...
$arrVida = array();
$query = "SELECT 
vademecum_listado.idVademecum, 
vademecum_listado.Titulo,
vademecum_categoria.Nombre AS Categoria

FROM `vademecum_listado`
LEFT JOIN `vademecum_categoria` ON vademecum_categoria.idCategoria = vademecum_listado.idCategoria
".$z."
...
<div class="col-lg-12">
    ...
    <?php foreach ($arrVida as $vida) { ?>
    	<tr class="odd">
			...
			<td>
<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$vida['idVademecum']; ?>" title="Editar Informacion" class="icon-editar info-tooltip"></a><?php }?>
<?php if ($rowlevel['level']>=4){?><?php $ubicacion=$location.'&del='.$vida['idVademecum'];?>			
