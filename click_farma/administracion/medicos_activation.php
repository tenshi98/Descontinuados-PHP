...
$original = "medicos_activation.php";
$location = $original;
...
if ( !empty($_POST['submit']) )  { 
	...
	$form_obligatorios = 'idCliente,idMedico,Fecha,Observacion';
	$form_trabajo= 'insert';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/medicos_observaciones.php';
}
		...
		<div id="div-1" class="body">	
			<form class="form-horizontal" method="post" name="form1">
				...
				<div class="form-group">
					...
					<?php $location .='&view_msg='.$_GET['view_msg'];?>
	...
	<div class="box dark">	
		...
		<div id="div-1" class="body">	
			<form class="form-horizontal" method="post" name="form1">
				...
				<div class="form-group">
					<input type="hidden" name="idMedico" value="<?php echo $_GET['view_msg'] ?>">
...
medicos_observaciones.Fecha,
medicos_observaciones.Observacion
FROM `medicos_observaciones`
LEFT JOIN `medicos_listado` usuario_observado   ON usuario_observado.idMedico     = medicos_observaciones.idMedico
LEFT JOIN `usuarios_listado` usuario_evaluador  ON usuario_evaluador.idUsuario     = medicos_observaciones.idUsuario
WHERE medicos_observaciones.idMedico = {$_GET['view']}
ORDER BY medicos_observaciones.idObservacion ASC ";
$resultado = mysql_query ($query, $dbConn);
$rowdata = mysql_fetch_assoc ($resultado);
?>
<div class="col-lg-8">
...
$query = "SELECT 
medicos_observaciones.idObservacion,
usuario_observado.Nombre AS nombre_cliente,
usuario_evaluador.Nombre AS nombre_usuario,
medicos_observaciones.Fecha,
medicos_observaciones.Observacion
...
LEFT JOIN `medicos_listado` usuario_observado   ON usuario_observado.idMedico     = medicos_observaciones.idMedico
LEFT JOIN `usuarios_listado` usuario_evaluador  ON usuario_evaluador.idUsuario     = medicos_observaciones.idUsuario
WHERE medicos_observaciones.idMedico = {$_GET['view_msg']}
ORDER BY medicos_observaciones.idObservacion ASC ";
$resultado = mysql_query ($query, $dbConn);
...
<div class="col-lg-12">
	<div class="box">	
		...
		<?php if ($rowlevel['level']>=1){?><a href="<?php echo $location.'&view_msg='.$_GET['view_msg'].'&view='.$observaciones['idObservacion']; ?>" title="Ver Informacion" class="icon-ver info-tooltip"></a><?php }?>   
		<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&view_msg='.$_GET['view_msg'].'&id='.$observaciones['idObservacion']; ?>" title="Editar Informacion" class="icon-editar info-tooltip"></a><?php }?>
		<?php if ($rowlevel['level']>=4){?><?php $ubicacion=$location.'&view_msg='.$_GET['view_msg'].'&del='.$observaciones['idObservacion'];?>			
...
$z="WHERE medicos_listado.idMedico>=0";
//Verifico el tipo de usuario que esta ingresando
if($arrUsuario['tipo']=='SuperAdmin'){
	$z.=" AND medicos_listado.idSistema>=0";
}else{
	$z.=" AND medicos_listado.idSistema={$arrUsuario['idSistema']}";	
}
//Verifico si la variable de busqueda existe
if (isset($_GET['search'])){
	$z.=" AND medicos_listado.Nombre LIKE '%{$_GET['search']}%' ";	
}	
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT idMedico FROM `medicos_listado` ".$z."";
...
medicos_listado.usuario,
medicos_listado.Nombre,
medicos_listado.Ultimo_acceso,
...
".$z."
ORDER BY  medicos_listado.usuario ASC
...
<div class="col-lg-12">
    ...
    <?php foreach ($arrUsers as $usuarios) { ?>
    	<tr class="odd">		
			<td><a name="<?php echo $usuarios['idMedico'] ?>"></a> <?php echo $usuarios['usuario']; ?></td>		
			...
			<?php //Verifico si existe la variable de busqueda y pagina
           	$w='&anclaje='.$usuarios['idMedico'];			?>  
            <?php if ($rowlevel['level']>=2){?>    				
            <ul class="interruptor">   
               <?php if ( $usuarios['estado']=='Activo' ) {?>   
                <li class="primer_int"><a href="<?php echo $location.'&id='.$usuarios['idMedico'].'&estado=2'.$w ; ?>">OFF</a></li>
                <li class="ultimo_int on"><a href="#">ON</a></li>
               <?php } else {?>
                <li class="primer_int on"><a href="#">OFF</a></li>
                <li class="ultimo_int"><a href="<?php echo $location.'&id='.$usuarios['idMedico'].'&estado=1'.$w ; ?>">ON</a></li>
               <?php }?>    
            </ul>
            <?php }?>       
            </td>
            <td class=" ">  
<?php if ($rowlevel['level']>=1){?><a href="<?php echo $location.'&view_msg='.$usuarios['idMedico']; ?>" title="Editar Observaciones" class="icon-msg info-tooltip"></a><?php }?>		
			</td>	
		</tr>
    <?php } ?>                    
	</tbody>
