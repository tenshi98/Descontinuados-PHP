...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			...
			case 'idMedico':       if(empty($idMedico)){        $error['idMedico']      = 'error/No ha seleccionado el cliente';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `medicos_observaciones` (idMedico, idUsuario, Fecha, Observacion) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				if(isset($idMedico) && $idMedico != ''){       $a .= ",idMedico='".$idMedico."'" ;}
				if(isset($idUsuario) && $idUsuario != ''){       $a .= ",idUsuario='".$idUsuario."'" ;}
				if(isset($Fecha) && $Fecha != ''){               $a .= ",Fecha='".$Fecha."'" ;}
				if(isset($Observacion) && $Observacion != ''){   $a .= ",Observacion='".$Observacion."'" ;}
		
				// inserto los datos de registro en la db
				$query  = "UPDATE `medicos_observaciones` SET ".$a." WHERE idObservacion = '$idObservacion'";
				$result = mysql_query($query, $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `medicos_observaciones` WHERE idObservacion = {$_GET['del']}";
			$result = mysql_query($query, $dbConn);	
