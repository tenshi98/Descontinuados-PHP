...
/*******************************************************************************************************************/
	...
	if ( !empty($_POST['idOfertasMias']) )   $idOfertasMias   = $_POST['idOfertasMias'];
...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			case 'idOfertasMias':  if(empty($idOfertasMias)){ $error['idOfertasMias']  = 'error/No ha ingresado el id';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Titulo)){
				$sql_Titulo = mysql_query("SELECT Titulo FROM ofertas_mias WHERE Titulo='".$Titulo."' "); 
				  ...
				  if (in_array($_FILES['imgLogo']['type'], $permitidos) && $_FILES['imgLogo']['size'] <= $limite_kb * 1024){
					...
					if (!file_exists($ruta)){
					  ...
					  if ($resultado){
							...
							$query  = "INSERT INTO `ofertas_mias` (Titulo, Descripcion, Valor, Direccion_img) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `ofertas_mias` SET ".$a." WHERE idOfertasMias = '$idOfertasMias'";
				$result = mysql_query($query, $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			if(unlink('upload/'.$archivo)){	
				...
				//se borran los datos	
				$query  = "DELETE FROM `ofertas_mias` WHERE idOfertasMias = {$_GET['del']}";
				$result = mysql_query($query, $dbConn);	
			  ...
			  if (in_array($_FILES['imgLogo']['type'], $permitidos) && $_FILES['imgLogo']['size'] <= $limite_kb * 1024){
				...
				if (!file_exists($ruta)){
				  ...
				  if ($resultado){
					...
					header( 'Location: '.$location.'&id_img='.$idOfertasMias );
					die;
...
/*******************************************************************************************************************/
		...
		case 'del_img':	
			...
			$query = "SELECT Direccion_img
			FROM `ofertas_mias`
			WHERE idOfertasMias = {$_GET['del_img']}";
			$resultado = mysql_query ($query, $dbConn);
			$rowdata = mysql_fetch_assoc ($resultado);
			...
			if(unlink('upload/'.$archivo)){	
				...
				$query  = "UPDATE `ofertas_mias` SET ".$a." WHERE idOfertasMias = '{$_GET['del_img']}'";
				$result = mysql_query($query, $dbConn);
				//Redirijo			
