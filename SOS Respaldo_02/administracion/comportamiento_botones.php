...
// Se trae un listado con todas las comunas
 if ($data['idGrupo']!=0&&$data['idGrupo']!=''){
	...
	WHERE idNotGrupo = ".$data['idGrupo']." ".$zy."
	ORDER BY Nombre ASC ";
	$resultado = mysql_query ($query, $dbConn);
	while ( $row = mysql_fetch_assoc ($resultado)) {
...
// Se trae un listado con todas las comunas
 if ($data['idGrupo']!=0&&$data['idGrupo']!=''){
	...
	WHERE idPagGrupo = ".$data['idGrupo']." ".$zy." 
	ORDER BY Nombre ASC ";
	$resultado = mysql_query ($query, $dbConn);
	while ( $row = mysql_fetch_assoc ($resultado)) {
	array_push( $arrCategoria,$row );
...
break;
//***********************************************************************************************************************************************//
//Se muestra el listado con todas las categorias de las preguntas
//***********************************************************************************************************************************************//
case 12:
...
<div class="form-group">
	...
	<div class="col-lg-8">
		<select name="idCategorias" class="form-control"   required>
			<option value="" selected="selected">Seleccione una Categorias</option>
			<?php foreach ($arrCategorias as $cat) { ?>
			<option value="<?php echo $cat['idCategorias']; ?>" <?php if(isset($idCategorias)&&$idCategorias==$cat['idCategorias']) {echo 'selected="selected"';}elseif($data['idCat']==$cat['idCategorias']){ echo 'selected="selected"'; } ?>><?php echo $cat['Nombre']; ?></option>
			<?php } ?>             
		</select>
	</div>
</div>					
...
// Se trae un listado con todas las comunas
 if ($data['idCat']!=0&&$data['idCat']!=''){
	$arrPreguntas = array();
	$query = "SELECT idPregunta, Pregunta
	FROM `preguntas_listado`
	".$z." AND idCategorias = ".$data['idCat']."  
...
// Se trae un listado de todas las categoria de las noticias
$query = "SELECT idPregunta, idCategorias, Pregunta
FROM `preguntas_listado` 
".$z."  ";
$resultado = mysql_query ($query, $dbConn);
while ($Preguntas[] = mysql_fetch_assoc ($resultado)); 
mysql_free_result($resultado);
array_pop($Preguntas);
array_multisort($Preguntas, SORT_ASC);
?>

<div class="form-group">
	<label for="text2" class="control-label col-lg-4">Categoria</label>
	<div class="col-lg-8">
		<select name="idCategorias" class="form-control"  onChange="cambia_preguntas()" required>
			<option value="" selected="selected">Seleccione una Categoria</option>
			<?php foreach ($arrCategorias as $cat) { ?>
			<option value="<?php echo $cat['idCategorias']; ?>" <?php if(isset($idCategorias)&&$idCategorias==$cat['idCategorias']) {echo 'selected="selected"';}elseif($data['idCat']==$cat['idCategorias']){ echo 'selected="selected"'; } ?>><?php echo $cat['Nombre']; ?></option>
			<?php } ?>             
...
<div class="form-group">
	<label for="text2" class="control-label col-lg-4">Preguntas</label>
	<div class="col-lg-8">
		<select name="idPregunta" class="form-control" required>
			<option value="" selected="selected">Seleccione una Pregunta</option>  
			<?php foreach ($arrPreguntas as $preg) { ?>
			<option value="<?php echo $preg['idPregunta']; ?>" <?php if(isset($idPregunta)&&$idPregunta==$preg['idPregunta']) {echo 'selected="selected"';} elseif($data['idListado']==$preg['idPregunta']){ echo 'selected="selected"'; }?>><?php echo $preg['Pregunta']; ?></option>
			<?php } ?>         
		</select>
	</div>
</div>
...
<?php
//se imprime la id de la tarea
filtrar($Preguntas, 'idCategorias'); 
foreach($Preguntas as $tipo=>$componentes){ 
echo'var id_noticia_'.$tipo.'=new Array(""';
foreach ($componentes as $idcomp) { 
echo',"'.$idcomp['idPregunta'].'"';
 }
 echo')
';
}
...
function cambia_preguntas(){
	var Componente
	Componente = document.form1.idCategorias[document.form1.idCategorias.selectedIndex].value
	try {
	if (Componente != '') {
		id_noticia=eval("id_noticia_" + Componente)
		noticia=eval("noticia_" + Componente)
		num_int = id_noticia.length
		document.form1.idPregunta.length = num_int
		for(i=0;i<num_int;i++){
		   document.form1.idPregunta.options[i].value=id_noticia[i]
		   document.form1.idPregunta.options[i].text=noticia[i]
		}	
	}else{
		document.form1.idPregunta.length = 1
		document.form1.idPregunta.options[0].value = ""
	    document.form1.idPregunta.options[0].text = "Seleccione una Noticia"
	}
	} catch (e) {
    alert("La categoria seleccionada no posee Noticias");
}
	document.form1.idPregunta.options[0].selected = true
}
</script>
<?php 
break;
