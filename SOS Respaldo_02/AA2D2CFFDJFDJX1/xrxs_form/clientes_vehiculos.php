...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			case 'idVehiculo':      if(empty($idVehiculo)){      $error['idVehiculo']       = 'error/No ha ingresado el id del sistema';}break;
			case 'idCliente':       if(empty($idCliente)){       $error['idCliente']        = 'error/No ha ingresado el idCliente del sistema';}break;
			case 'PPU':             if(empty($PPU)){             $error['PPU']              = 'error/No ha ingresado la imagen';}break;
			case 'idMarca':         if(empty($idMarca)){         $error['idMarca']          = 'error/No ha ingresado el email';}break;
			case 'idModelo':        if(empty($idModelo)){        $error['idModelo']         = 'error/No ha ingresado la razon social';}break;
			case 'idTransmision':   if(empty($idTransmision)){   $error['idTransmision']    = 'error/No ha ingresado el idTransmision';}break;
			case 'fTransferencia':  if(empty($fTransferencia)){  $error['fTransferencia']   = 'error/No ha ingresado la fTransferencia';}break;
			...
			case 'N_Motor':         if(empty($N_Motor)){         $error['N_Motor']          = 'error/No ha ingresado el N_Motor';}break;	
			case 'N_Chasis':        if(empty($N_Chasis)){        $error['N_Chasis']         = 'error/No ha ingresado la carpeta de imagenes';}break;
			case 'Obs':             if(empty($Obs)){             $error['Obs']              = 'error/No ha ingresado la carpeta de mp3';}break;
