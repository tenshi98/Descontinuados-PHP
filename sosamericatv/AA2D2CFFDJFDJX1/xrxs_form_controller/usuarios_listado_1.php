...
//Se validan si existe el usuario o el email asociado de este en la base de datos
	...
	if(isset($usuario)){
		$sql_usuario = mysql_query("SELECT usuario FROM usuarios_listado WHERE usuario='".$usuario."' AND mode='1' "); 
		$n_usr = mysql_num_rows($sql_usuario);
	} else {$n_usr=0;}
	// se verifica si el correo ya existe
	if(isset($email)){
		$sql_email = mysql_query("SELECT email FROM usuarios_listado WHERE email='".$email."' AND mode='1' ");
