...
if ( !empty($_POST['submit']) )  { 
	...
	$form_obligatorios = 'Titulo,Descripcion,Fono';
	$form_trabajo= 'insert';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/ofertas_diarias.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$form_obligatorios = 'idOfertasDia,Titulo,Descripcion,Fono';
	$form_trabajo= 'update';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/ofertas_diarias.php';
}
//se borra un dato
...
WHERE idOfertasDia = {$_GET['id_img']}";
		...
		<?php }else{?>
			...
			<form class="form-horizontal" method="post" enctype="multipart/form-data">
				...
				<div class="form-group">
					<input type="hidden" name="idOfertasDia" value="<?php echo $_GET['id_img']; ?>">
...
// Se traen todos los datos de mi usuario
$query = "SELECT Titulo, Descripcion, Fono
FROM `ofertas_diarias`
WHERE idOfertasDia = {$_GET['id']}";
$resultado = mysql_query ($query, $dbConn);
$rowdata = mysql_fetch_assoc ($resultado);	?>
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			if(isset($Fono)) {          $x3  = $Fono;          }else{$x3  = $rowdata['Fono'];}

			//se dibujan los inputs
			echo form_input('text', 'Titulo', 'Titulo', $x1, 2); 
			echo form_textarea('Descripcion', 'Descripcion', $x2, 2);
			echo form_input_phone('Fono', 'Fono', $x3, 2); 
			?>
          
			<div class="form-group">
            	<input type="hidden" name="idOfertasDia" value="<?php echo $_GET['id']; ?>" >
...
$arrOfertas = array();
$query = "SELECT idOfertasDia, Titulo
FROM `ofertas_diarias`
".$z."
ORDER BY Titulo ASC
...
<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id_img='.$ofertas['idOfertasDia']; ?>" title="Editar Imagen" class="icon-editar-img info-tooltip"></a><?php }?>
<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$ofertas['idOfertasDia']; ?>" title="Editar Informacion" class="icon-editar info-tooltip"></a><?php }?>
<?php if ($rowlevel['level']>=4){?><?php $ubicacion=$location.'&del='.$ofertas['idOfertasDia'];?>			
