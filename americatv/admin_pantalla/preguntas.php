...
if ( !empty($_POST['submit_create']) )  { 
	...
	require_once 'AA2D2CFFDJFDJX1/xrxs_form_common/preguntas.php';
...
//Trae un listado con todas las interfaces
$arrInterfaz = array();
$query = "SELECT idInterfaz, Nombre
FROM interfaz
ORDER BY Nombre ASC ";
$resultado = mysql_query ($query, $dbConn);
while ( $row = mysql_fetch_assoc ($resultado)) {
...
<form method="post">
<tr>
	<td width="19%"height="40" align="left" valign="middle"><span class="arial_tit_gris">Interfaz</span></td>
	<td>
    <select name="idInterfaz" required="required" class="arial_light_cuadro">
		<option value='' >Seleccione una Interfaz</option>  
		<?php foreach ($arrInterfaz as $interfaz) { ?>
    	<option value="<?php echo $interfaz['idInterfaz']; ?>" <?php if(isset($idInterfaz)&&$idInterfaz==$interfaz['idInterfaz']) {echo 'selected="selected"';}elseif($row_data['idInterfaz']==$interfaz['idInterfaz']){echo 'selected="selected"';}?>><?php echo $interfaz['Nombre']; ?></option>
    <?php } ?>
	</select>
    </td>
</tr>
...
//Trae un listado con todas las categorias de las preguntas
$arrCategorias = array();
$query = "SELECT
preguntas_categorias.idCatPreg,
preguntas_categorias.Categoria
...
<tr>
    ...
    <select name="idInterfaz" required="required" class="arial_light_cuadro">
		<option value='' >Seleccione una Interfaz</option>  
		<?php foreach ($arrInterfaz as $interfaz) { ?>
    	<option value="<?php echo $interfaz['idInterfaz']; ?>" <?php if(isset($idInterfaz)&&$idInterfaz==$interfaz['idInterfaz']) {echo 'selected="selected"';}?>><?php echo $interfaz['Nombre']; ?></option>
    <?php } ?>
	</select>
    </td>
</tr>
...
interfaz.Nombre AS nombre_interfaz,
preguntas_categorias.Categoria AS categoria_pregunta
FROM preguntas
LEFT JOIN interfaz on interfaz.idInterfaz = preguntas.idInterfaz
LEFT JOIN preguntas_categorias on preguntas_categorias.idCatPreg = preguntas.idCatPreg
...
?>
	...
	<tr>
        ...
        <span class="arial_tit_gris"><?php echo $pregunta['nombre_interfaz'].' - '.$pregunta['categoria_pregunta']; ?></span><br />
        <span class="arial_tit_gris"><?php echo $pregunta['Pregunta']; ?></span><br />
		<span class="arial_cuerpo_gris"><?php echo $pregunta['Opcion1'].'('.porcentaje_cd($por_opcion1).') - '.$pregunta['Opcion2'].'('.porcentaje_cd($por_opcion2).')'; ?></span>
        <br />
        </td>
         <td width="19%">
         	<?php 
         ...
         <td width="19%">
            <a class="btn_list btn-app" href="<?php echo $location ?>?id=<?php echo $pregunta['idPregunta'] ?>"><i class="fap fap-edit"></i> Edit</a>
