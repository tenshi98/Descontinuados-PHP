...
if ( !empty($_POST['submit']) )  {
	...
	if ( !empty($_POST['ciudad_ejecutiv']) )    $dir_ejecutiv      = $_POST['dir_ejecutiv'];
...
$juntos=$uno.$dos;		
		...
		$query  = "INSERT INTO `ejecutivos` (nom_ejecutiv, fono_ejecutiv, ciudad_ejecutiv, login,pass, fecha_ingreso, mail_ejecutiv, grupo, radio, publicidad, estado_usr, codigo_sms,gcmcode,imei,lon,lat,dir_ejecutiv,rut) VALUES ('{$nom_ejecutiv}','{$telefono}','{$ciudad_ejecutiv}','{$login}','1234',{$fecha_ing},'{$login}','{$grupo}', {$radio}, '{$publicidad}', '{$estado_usr}', '{$codigo_sms}', '{$gcmcode}', '{$imei}', {$longitud}, {$latitud}, '{$dir_ejecutiv}','{$juntos}'  )";
...
$sql ="INSERT INTO padron_electoral_CL (CIDENTIDAD, DV_CEDID, rut_compara, NOMBRE, SEX, DOMICILIO, CIRCELECTORAL, CIRCUNSCRIPCION, Comuna, PROVINCIA, REGION, GLOSA_REGION, MESA, NROMESA, TIPOMESA, distrito, csenatorial, GLOSASENATORIAL, codi_fijo, fono_fijo, fono_celular, filtro, filtro2, actualizacion, colegiovotac, dircolegiovotac, domicilio_2, comuna_2, tipo_domicilio2, correo) VALUES ('{$uno}', '{$dos}', '{$juntos}', '{$nom_ejecutiv}', 'ND', '{$dir_ejecutiv}', '0', '{$ciudad_ejecutiv}', '{$ciudad_ejecutiv}', '{$ciudad_ejecutiv}', 'ND', 'ND', 'ND', 0, 'ND', 'ND', 'ND', 'ND', 0, 0, '{$telefono}', '', ' ', '0', 'ND', 'ND', '{$dir_ejecutiv}', '{$ciudad_ejecutiv}', 'RES', '{$login}')";
		...
		//$sql ="update padron_electoral_CL set domicilio_2='{$dir_ejecutiv}', comuna_2='{$ciudad_ejecutiv}', tipo_domicilio2='RES', correo='{$login}',fono_celular='{$fono_ejecutiv}'  WHERE rut_compara='" . $_POST['rut_compara'] . "'";
