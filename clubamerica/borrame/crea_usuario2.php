...
$fecha_nacimiento=$anio.$mes_dis.$dia_dis;
	...
	if ( empty($ciudad_ejecutiv) )				$errors[12] 	  = '<div class="alert-error" >No ha ingresado su Distrito</div>';
	...
	if ( empty($idDepartamento) )				$errors[19] 	  = '<div class="alert-error" >No ha ingresado el Departamento</div>';
	if ( empty($region) )						$errors[20] 	  = '<div class="alert-error" >No ha ingresado La Provincia</div>';



	if ( !isset($errors[1]) && !isset($errors[2]) && !isset($errors[3])  && !isset($errors[5]) && !isset($errors[6]) && !isset($errors[7]) && !isset($errors[8]) && !isset($errors[9]) && !isset($errors[10]) && !isset($errors[11]) && !isset($errors[12]) && !isset($errors[13]) && !isset($errors[14]) && !isset($errors[15]) && !isset($errors[16]) && !isset($errors[17]) && !isset($errors[18]) && !isset($errors[19]) && !isset($errors[20])) {
		...
		$query  = "INSERT INTO `ejecutivos` (nom_ejecutiv, fono_ejecutiv, departamento,region,ciudad_ejecutiv, login,pass, fecha_ingreso, mail_ejecutiv, grupo, radio, publicidad, estado_usr, codigo_sms,gcmcode,imei,lon,lat,dir_ejecutiv,rut,dispositivo,sexo,fecha_nac) VALUES ('{$nom_ejecutiv}','{$telefono}','{$idDepartamento}','{$region}','{$ciudad_ejecutiv}','{$login}','1234',{$fecha_ing},'{$login}','{$grupo}', {$radio}, '{$publicidad}', '{$estado_usr}', '{$codigo_sms}', '{$gcmcode}', '{$imei}', {$longitud}, {$latitud}, '{$dir_ejecutiv}','{$juntos}','{$dispositivo}','{$sexo}','{$fecha_nacimiento}'  )";
...
if ($conexion_padron==0) {
		...
		$query_2  = "INSERT INTO `ejecutivos_gral` (id_ejecutiv,nom_ejecutiv, fono_ejecutiv, departamento,region, ciudad_ejecutiv, login,pass, fecha_ingreso, mail_ejecutiv, grupo, radio, publicidad, estado_usr, codigo_sms,gcmcode,imei,lon,lat,dir_ejecutiv,rut,dispositivo,sexo,fecha_nac) VALUES ({$result_id_dato},'{$nom_ejecutiv}','{$telefono}','{$idDepartamento}','{$region}','{$ciudad_ejecutiv}','{$login}','1234',{$fecha_ing},'{$login}','{$grupo}', {$radio}, '{$publicidad}', '{$estado_usr}', '{$codigo_sms}', '{$gcmcode}', '{$imei}', {$longitud}, {$latitud}, '{$dir_ejecutiv}','{$juntos}','{$dispositivo}','{$sexo}','{$fecha_nacimiento}'  )";
...
<div class="height100 widht100">
    <div class="widht80 fcenter perfil">
        ...
        <?php  if (isset($errors[18])) {echo $errors[19];}?>
        <?php  if (isset($errors[18])) {echo $errors[20];}?>
        <h1>Crear usuario</h1>
...
<tr><td><input type="tel" name="fono_ejecutiv" placeholder="Tel&eacute;fono Movil" autocomplete="off" <?php if(isset($fono_ejecutiv)) {echo 'value="'.$fono_ejecutiv.'"';}?>></td></tr> 
...
<tr>
    ...
    <option <?php  if (isset($idDepartamento)) {echo "value='".$idDepartamento."'"; }else{echo "value=''"; }?> selected="selected">
	<?php  if (isset($idDepartamento)) {
		$queryubicacion = "SELECT Nombre from ubicacion_departamento WHERE idDepartamento = '".$idDepartamento."'";
		$resultadoubicacion = mysql_query ($queryubicacion, $dbConn);
		$rowubicacion = mysql_fetch_assoc ($resultadoubicacion); 
		echo $rowubicacion["Nombre"]; 
		}else{
...
<tr>
    ...
    <select class="ciudad_ejecutiv" name="idProvincia" onChange="cambia_provincia()" >
        <option <?php  if (isset($region)) {echo "value='".$region."'"; }else{echo "value=''"; }?> selected="selected">
	<?php  if (isset($region)) {
		$queryubicacion = "SELECT Nombre from ubicacion_provincia WHERE idProvincia = '".$region."'";
		$resultadoubicacion = mysql_query ($queryubicacion, $dbConn);
		$rowubicacion = mysql_fetch_assoc ($resultadoubicacion); 
		echo $rowubicacion["Nombre"]; 
		}else{
...
<tr>
    ...
    <select class="ciudad_ejecutiv" name="idDistrito"  >
            <option <?php  if (isset($ciudad_ejecutiv)) {echo "value='".$ciudad_ejecutiv."'"; }else{echo "value=''"; }?> selected="selected">
	<?php  if (isset($ciudad_ejecutiv)) {
		$queryubicacion = "SELECT Nombre from ubicacion_distrito WHERE idDistrito = '".$ciudad_ejecutiv."'";
		$resultadoubicacion = mysql_query ($queryubicacion, $dbConn);
		$rowubicacion = mysql_fetch_assoc ($resultadoubicacion); 
		echo $rowubicacion["Nombre"]; 
		}else{
			echo "Seleccione un Distrito"; }
			?> 
	</option>                            
    </select>   
    </td>
