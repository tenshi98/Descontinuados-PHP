...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			case 'idAdmpm':       if(empty($idAdmpm)){       $error['idAdmpm']         = 'error/No ha ingresado el id del permiso';}break;
			case 'id_pmcat':      if(empty($id_pmcat)){      $error['id_pmcat']        = 'error/No ha seleccionado una categoria';}break;
			case 'Direccionbase': if(empty($Direccionbase)){ $error['Direccionbase']   = 'error/No ha ingresado el nombre del archivo';}break;
			case 'Nombre':        if(empty($Nombre)){        $error['Nombre']          = 'error/No ha ingresado el nombre a mostrar';}break;
			case 'visualizacion': if(empty($visualizacion)){ $error['visualizacion']   = 'error/No ha seleccionado quien ve la transaccion';}break;
		}
	}
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if($n1 > 0) {$error['Nombre'] = 'error/El permiso ya existe';}
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `core_permisos` ( id_pmcat, Direccionbase, Nombre, visualizacion ) VALUES ({$a} )";
