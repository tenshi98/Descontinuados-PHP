...
if ( !empty($_POST['submit']) )  { 
	...
	$form_trabajo= 'insert';
	require_once '../XRXS_2D2CFFDJFDJX1/xrxs_form/mnt_ubicacion_pais.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$form_obligatorios = 'Nombre,idPais';
	$form_trabajo= 'update';
	require_once '../XRXS_2D2CFFDJFDJX1/xrxs_form/mnt_ubicacion_pais.php';
}
//se borra un dato
...
if (isset($_GET['created'])) {$error['usuario'] 	  = 'sucess/Pais creado correctamente';}
if (isset($_GET['edited']))  {$error['usuario'] 	  = 'sucess/Pais editado correctamente';}
if (isset($_GET['deleted'])) {$error['usuario'] 	  = 'sucess/Pais borrado correctamente';}
...
$query = "SELECT Nombre
FROM `mnt_ubicacion_pais`
WHERE idPais = {$_GET['id']}";
...
$query = "SELECT idPais FROM `mnt_ubicacion_pais` ".$z."";
...
<label class="control-label col-lg-4">Buscar Pais</label>
    <div class="col-lg-5">
...
<?php if ($rowlevel['level_crear']==1){?><a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width" >Crear Nuevo Pais</a><?php } ?>
</div>
...
<div class="col-lg-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Paises</h5>
		</header>
    ...
    <?php foreach ($arrCuentas as $cuentas) { ?>
    	<tr class="odd">
			...
			<td>
<?php if ($rowlevel['level_editar']==1){?><a href="<?php echo $location.'&id='.$cuentas['idPais']; ?>" title="Editar Informacion" class="icon-editar info-tooltip"></a><?php } ?>
<?php if ($rowlevel['level_borrar']==1){?><?php $ubicacion=$location.'&del='.$cuentas['idPais'];?>
