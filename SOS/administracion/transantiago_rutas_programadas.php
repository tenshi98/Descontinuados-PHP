...
$original = "transantiago_rutas_programadas.php";
$location = $original;
//Se agregan ubicaciones
$location .='?pagina='.$_GET['pagina'];
if(isset($_GET['search']) && $_GET['search'] != ''){             $location .= "&search=".$_GET['search'] ; 	}
if(isset($_GET['idEstado']) && $_GET['idEstado'] != ''){         $location .= "&idEstado=".$_GET['idEstado'] ; }
if(isset($_GET['idDias']) && $_GET['idDias'] != ''){             $location .= "&idDias=".$_GET['idDias'] ; }
...
if(isset($_GET['HoraInicio']) && $_GET['HoraInicio'] != ''){     $location .= "&HoraInicio=".$_GET['HoraInicio'] ; }
if(isset($_GET['HoraTermino']) && $_GET['HoraTermino'] != ''){   $location .= "&HoraTermino=".$_GET['HoraTermino'] ; }
...
if ( !empty($_POST['submit']) )  { 
	...
	$form_obligatorios = 'idDias,idRecorrido,idRuta,HoraInicio,HoraTermino,Nombre';
...
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$location.='&id='.$_GET['id'];
	$form_obligatorios = 'idRutaAlt,idEstado,idDias,idRecorrido,idRuta,HoraInicio,HoraTermino,Nombre';
	$form_trabajo= 'update';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/transantiago_rutas_multi.php';
}
//se borra un dato
...
if (isset($_GET['created'])) {$error['usuario'] 	  = 'sucess/Ruta Programada creada correctamente';}
if (isset($_GET['edited']))  {$error['usuario'] 	  = 'sucess/Ruta Programada editada correctamente';}
if (isset($_GET['deleted'])) {$error['usuario'] 	  = 'sucess/Ruta Programada borrada correctamente';}
...
<div class="col-lg-4 fleft">
	<div class="box dark">
		<header>
			...
			<h5>Modificacion datos de Ruta Programada</h5>
		</header>
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" name="form1">
				...
				if(isset($idDias)) {        $x2  = $idDias;        }else{$x2  = $rowdata['idDias'];}
				if(isset($idRecorrido)) {   $x3  = $idRecorrido;   }else{$x3  = $rowdata['idRecorrido'];}
				if(isset($idRuta)) {        $x4  = $idRuta;        }else{$x4  = $rowdata['idRuta'];}
				...
				echo form_select('Dias','idDias', $x2, 2, 'idDias', 'Nombre', 'transantiago_rutas_multi_dias', 0, $dbConn);
				echo form_select('Recorrido','idRecorrido', $x3, 2, 'idRecorrido', 'Nombre', 'transantiago_recorridos', 0, $dbConn);
				echo form_select('Ruta','idRuta', $x4, 2, 'idRuta', 'Nombre', 'transantiago_rutas', 0, $dbConn);	
				echo form_time('Hora Inicio','HoraInicio', $x5, 2);	
				echo form_time('Hora Termino','HoraTermino', $x6, 2);	
				echo form_textarea('Descripcion', 'Nombre', $x7, 2); 
				?>
			...
			<script>
				...
				function RutasAlternativas() {
					...
					<?php foreach ( $arrRutasAlt as $pos ) { ?>
						['<?php echo $pos['idListado']; ?>', <?php echo $pos['Latitud']; ?>, <?php echo $pos['Longitud']; ?>], 					
					<?php } ?>
					];
...
<?php ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
 ...
 <div class="col-lg-6 fcenter">
	<div class="box dark">
		<header>
			<div class="icons"><i class="fa fa-edit"></i></div>
			<h5>Crear Nueva Ruta Programada</h5>
		</header>
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" name="form1">
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idDias)) {        $x1  = $idDias;        }else{$x1  = '';}
				...
				//se dibujan los inputs
				echo form_select('Dias','idDias', $x1, 2, 'idDias', 'Nombre', 'transantiago_rutas_multi_dias', 0, $dbConn);
				...
				echo form_time('Hora Inicio','HoraInicio', $x4, 2);	
				echo form_time('Hora Termino','HoraTermino', $x5, 2);	
				echo form_textarea('Descripcion', 'Nombre', $x6, 2); 
				?>
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form name="form1" class="form-horizontal" action="<?php echo $location; ?>" >
				...
				if(isset($idDias)) {        $x3  = $idDias;        }else{$x3  = '';}
				if(isset($idRecorrido)) {   $x4  = $idRecorrido;   }else{$x4  = '';}
				if(isset($idRuta)) {        $x5  = $idRuta;        }else{$x5  = '';}
				if(isset($HoraInicio)) {    $x7  = $HoraInicio;    }else{$x7  = '';}
				if(isset($HoraTermino)) {   $x8  = $HoraTermino;   }else{$x8  = '';}
				if(isset($Nombre)) {        $x9  = $Nombre;        }else{$x9  = '';}

				//se dibujan los inputs
				echo form_select('Estado','idEstado', $x1, 1, 'idEstado', 'Nombre', 'transantiago_rutas_multi_estado', 0, $dbConn);
				echo form_select('Dias','idDias', $x3, 1, 'idDias', 'Nombre', 'transantiago_rutas_multi_dias', 0, $dbConn);
				echo form_select('Recorrido','idRecorrido', $x4, 1, 'idRecorrido', 'Nombre', 'transantiago_recorridos', 0, $dbConn);
				echo form_select('Ruta','idRuta', $x5, 1, 'idRuta', 'Nombre', 'transantiago_rutas', 0, $dbConn);
				echo form_time('Hora Inicio','HoraInicio', $x7, 1);	
				echo form_time('Hora Termino','HoraTermino', $x8, 1);	
				echo form_textarea('Descripcion', 'Nombre', $x9, 1); 
				?>        
...
$z="WHERE transantiago_rutas_multi.idTipo = 2";	
...
transantiago_rutas_multi_dias.Nombre AS dia,
...
<?php if ($rowlevel['level']>=3){?><a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width" >Crear Nueva Ruta Prog</a><?php }?>
</div>
<div class="clearfix"></div>                     
...
<div class="col-lg-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Rutas Programadas</h5>
		</header>
    ...
    <?php foreach ($arrRutas as $rutas) { ?>
    	<tr class="odd">
			<td><?php echo 'Todos los '.$rutas['dia'].' de '.$rutas['HoraInicio'].' a '.$rutas['HoraTermino']; ?></td>
