...
if ( !empty($_POST['submit']) )  { 
	...
	$form_obligatorios = 'PPU,idMarca,idModelo,idTransmision,idColorV_1,fFabricacion,N_Motor,N_Chasis';
	$form_trabajo= 'insert';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/clientes_vehiculos.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$location.='&view='.$_GET['view'];
	$form_obligatorios = 'idVehiculo,PPU,idMarca,idModelo,idTransmision,idColorV_1,fFabricacion,N_Motor,N_Chasis';
	$form_trabajo= 'update';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/clientes_vehiculos.php';
}
//se borra un dato
      ...
      <div id="top">
        ...
        <header class="head">
          <div class="main-bar">
            ...
            <i class="fa fa-dashboard"></i> <?php echo $rowlevel['nombre_transaccion']; ?>		<!-- InstanceEndEditable --> </h3>
          </div>
        </header>
      </div>
      <div id="left">
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		<header>
			...
			<h5>Modificacion datos del Vehiculo</h5>
		</header>
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			if(isset($PPU)) {             $x1  = $PPU;             }else{$x1  = $rowdata['PPU'];}
            if(isset($idMarca)) {         $x2  = $idMarca;         }else{$x2  = $rowdata['idMarca'];}
            if(isset($idModelo)) {        $x3  = $idModelo;        }else{$x3  = $rowdata['idModelo'];}
			if(isset($idTransmision)) {   $x4  = $idTransmision;   }else{$x4  = $rowdata['idTransmision'];}		
			if(isset($idColorV_1)) {      $x5  = $idColorV_1;      }else{$x5  = $rowdata['idColorV_1'];}
			if(isset($idColorV_2)) {      $x6  = $idColorV_2;      }else{$x6  = $rowdata['idColorV_2'];}
            if(isset($fTransferencia)) {  $x7  = $fTransferencia;  }else{$x7  = $rowdata['fTransferencia'];}
			if(isset($fFabricacion)) {    $x8  = $fFabricacion;    }else{$x8  = $rowdata['fFabricacion'];}
			if(isset($N_Motor)) {         $x9  = $N_Motor;         }else{$x9  = $rowdata['N_Motor'];}
			if(isset($N_Chasis)) {        $x10 = $N_Chasis;        }else{$x10 = $rowdata['N_Chasis'];}
			if(isset($Obs)) {             $x11 = $Obs;             }else{$x11 = $rowdata['Obs'];}	

			//se dibujan los inputs
			echo form_input('text', 'Patente', 'PPU', $x1, 2);
			echo form_select_depend1('Marca','idMarca', $x2, 2, 'idMarca', 'Nombre', 'vehiculos_marcas', 0,
									'Modelo','idModelo', $x3, 1, 'idModelo', 'idMarca', 'Nombre', 'vehiculos_modelos', 0, 
									 $dbConn);
			echo form_select('Tipo de Transmision','idTransmision', $x4, 2, 'idTransmision', 'Nombre', 'vehiculos_transmision', 0, $dbConn);
			echo form_select('Color Base','idColorV_1', $x5, 2, 'idColorV', 'Nombre', 'vehiculos_colores', 0, $dbConn);
			echo form_select('Color Complementario','idColorV_2', $x6, 1, 'idColorV', 'Nombre', 'vehiculos_colores', 0, $dbConn);
			echo form_date('Fecha de Transferencia','fTransferencia', $x7, 1);
			echo form_date('Fecha de Fabricacion','fFabricacion', $x8, 2);
			echo form_input('text', 'Numero de Motor', 'N_Motor', $x9, 2);
			echo form_input('text', 'Numero de Chasis', 'N_Chasis', $x10,2);
			echo form_obs_simple('Observaciones', 'Obs', $x11, 1);
			?>
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
            ...
            <?php 
			//Se verifican si existen los datos
			if(isset($PPU)) {             $x1  = $PPU;             }else{$x1  = '';}
            if(isset($idMarca)) {         $x2  = $idMarca;         }else{$x2  = '';}
            if(isset($idModelo)) {        $x3  = $idModelo;        }else{$x3  = '';}
			if(isset($idTransmision)) {   $x4  = $idTransmision;   }else{$x4  = '';}	
			if(isset($idColorV_1)) {      $x5  = $idColorV_1;      }else{$x5  = '';}
			if(isset($idColorV_2)) {      $x6  = $idColorV_2;      }else{$x6  = '';}
            if(isset($fTransferencia)) {  $x7  = $fTransferencia;  }else{$x7  = '';}
			if(isset($fFabricacion)) {    $x8  = $fFabricacion;    }else{$x8  = '';}
			if(isset($N_Motor)) {         $x9  = $N_Motor;         }else{$x9  = '';}
			if(isset($N_Chasis)) {        $x10 = $N_Chasis;        }else{$x10 = '';}
			if(isset($Obs)) {             $x11 = $Obs;             }else{$x11 = '';}
...
<div class="col-lg-6">
	<div class="box">	
        ...
        <div class="body">
            ...
            <p class="text-muted"><strong>Observaciones : </strong><?php echo $rowdata['observaciones']; ?></p>		<?php }?>
        </div></div>
</div>
...
<div class="col-lg-12">
    ...
    <?php foreach ($arrVehiculos as $vehiculos) { ?>
    	<tr class="odd">		
            ...
            <td class=" "><?php echo $vehiculos['fecha_fabricacion']; ?></td>		<td class=" ">
