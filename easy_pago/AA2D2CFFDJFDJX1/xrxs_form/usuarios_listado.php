...
/*******************************************************************************************************************/
	...
	if ( !empty($_POST['Comision']) )       $Comision       = $_POST['Comision'];
	if ( !empty($_POST['idCreador']) )      $idCreador      = $_POST['idCreador'];
...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			...
			case 'Comision':       if(empty($Comision)){      $error['Comision']       = 'error/No ha ingresado la comision';}break;
			case 'idCreador':      if(empty($idCreador)){     $error['idCreador']      = 'error/No ha ingresado el creador';}break;
		}
	}
/*******************************************************************************************************************/
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if ( empty($error) ) {
				//Paso todos los caracteres a minusculas
				...
				if(isset($Comision) && $Comision != ''){            $a .= ",'".$Comision."'" ;       }else{$a .= ",''";}
				if(isset($idCreador) && $idCreador != ''){          $a .= ",'".$idCreador."'" ;      }else{$a .= ",''";}
				
				// inserto los datos de registro en la db
				$query  = "INSERT INTO `usuarios_listado` (usuario, password, tipo, idEstado, email, Nombre, Rut, fNacimiento, Direccion, 
				Fono, Ciudad, Comuna, Direccion_img, Ultimo_acceso, idSistema, Comision, idCreador) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				if(isset($Comision) && $Comision != ''){            $a .= ",Comision='".$Comision."'" ;}
				if(isset($idCreador) && $idCreador != ''){          $a .= ",idCreador='".$idCreador."'" ;}
...
/*******************************************************************************************************************/		
		case 'insert_admin':
			...
			if ( empty($error) ) {
				...
				//Agrego los permisos de forma manual
				$id_usuario = $ultimo_id;
				//Tiene permisos para crear y editar vendedores
				$query  = "INSERT INTO `usuarios_permisos` (idUsuario, idAdmpm, level)VALUES ('$id_usuario','17','4')";
				$result = mysql_query($query, $dbConn);
				$query  = "INSERT INTO `usuarios_permisos` (idUsuario, idAdmpm, level)VALUES ('$id_usuario','18','4')";
				$result = mysql_query($query, $dbConn);
				$query  = "INSERT INTO `usuarios_permisos` (idUsuario, idAdmpm, level)VALUES ('$id_usuario','19','4')";
				$result = mysql_query($query, $dbConn);
				//Tiene permisos para crear y editar bodegas
				/*$query  = "INSERT INTO `usuarios_permisos` (idUsuario, idAdmpm, level)VALUES ('$id_usuario','1','4')";
				$result = mysql_query($query, $dbConn);
				$query  = "INSERT INTO `usuarios_permisos` (idUsuario, idAdmpm, level)VALUES ('$id_usuario','2','4')";
				$result = mysql_query($query, $dbConn);
				$query  = "INSERT INTO `usuarios_permisos` (idUsuario, idAdmpm, level)VALUES ('$id_usuario','13','4')";
				$result = mysql_query($query, $dbConn);
				*/
				//Se agregan los permisos a los informes del mes
				$query  = "INSERT INTO `usuarios_permisos` (idUsuario, idAdmpm, level)VALUES ('$id_usuario','21','4')";
				$result = mysql_query($query, $dbConn);
...
/*******************************************************************************************************************/		
		case 'insert_vendedor':
			...
			if ( empty($error) ) {
				...
				$result = mysql_query($query, $dbConn);
				//Se agregan los permisos a los informes del mes
				$query  = "INSERT INTO `usuarios_permisos` (idUsuario, idAdmpm, level)VALUES ('$id_usuario','22','4')";
				$result = mysql_query($query, $dbConn);
