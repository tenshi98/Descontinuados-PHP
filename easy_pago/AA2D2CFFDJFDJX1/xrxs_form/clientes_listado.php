...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			...
			case 'idDepartamento':    if(empty($idDepartamento)){    $error['idDepartamento']     = 'error/No ha ingresado el gsm';}break;
			case 'idProvincia':       if(empty($idProvincia)){       $error['idProvincia']        = 'error/No ha ingresado el gsm';}break;
			case 'idDistrito':        if(empty($idDistrito)){        $error['idDistrito']         = 'error/No ha ingresado el gsm';}break;
		}
	}
/*******************************************************************************************************************/
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if ( empty($error) ) {
				...
				$password        = '1234';        //Encripto la clave
				...
				$query  = "INSERT INTO `clientes_listado` (idSistema, idEstado, idTipo, email, Nombre,Rut, fNacimiento, Direccion, Fono1, Fono2, Departamento, 
				Provincia, Distrito, unique_id, password, fcreacion, factualizacion, IMEI, GSM, Latitud, Longitud, dispositivo, Saldo, idUsuario,
				Rubro) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'insert2':
			...
			WHERE idProvincia = {$idProvincia} AND idDepartamento = {$idDepartamento}";
			$resultado = mysql_query ($query, $dbConn);
			...
			WHERE idDistrito = {$idDistrito} AND idProvincia = {$idProvincia}";
			$resultado = mysql_query ($query, $dbConn);
			...
			if ( empty($error) ) {
				...
				if(isset($Fono1) && $Fono1 != ''){                         $a .= ",'".$Fono1."'" ;             }else{$a .= ",''";}
				if(isset($Fono2) && $Fono2 != ''){                         $a .= ",'".$Fono2."'" ;             }else{$a .= ",''";}
				if(isset($rowdepto['Nombre'])&&$rowdepto['Nombre'] != ''){ $a .= ",'".$rowdepto['Nombre']."'"; }else{$a .= ",''";}
				if(isset($rowprov['Nombre'])&&$rowprov['Nombre'] != ''){   $a .= ",'".$rowprov['Nombre']."'";  }else{$a .= ",''";}
				if(isset($rowdist['Nombre'])&&$rowdist['Nombre'] != ''){   $a .= ",'".$rowdist['Nombre']."'";  }else{$a .= ",''";}
				if(isset($unique_id) && $unique_id != ''){                 $a .= ",'".$unique_id."'" ;         }else{$a .= ",''";}
				if(isset($password) && $password != ''){                   $a .= ",'".md5($password)."'" ;     }else{$a .= ",''";}
				if(isset($fcreacion) && $fcreacion != ''){                 $a .= ",'".$fcreacion."'" ;         }else{$a .= ",''";}
				if(isset($factualizacion) && $factualizacion != ''){       $a .= ",'".$factualizacion."'" ;    }else{$a .= ",''";}
...
/*******************************************************************************************************************/		
		case 'update2':	
			...
			if ( empty($error) ) {
				...
				if(isset($rowdepto['Nombre'])&&$rowdepto['Nombre'] != ''){ $a .= ",Departamento='".$rowdepto['Nombre']."'" ;}
				if(isset($rowprov['Nombre'])&&$rowprov['Nombre'] != ''){   $a .= ",Provincia='".$rowprov['Nombre']."'" ;}
				if(isset($rowdist['Nombre'])&&$rowdist['Nombre'] != ''){   $a .= ",Distrito='".$rowdist['Nombre']."'" ;}
