...
<script>
  ...
  function connect(c) {
    ...
    conn.on('data', function(data){
      $('#messages').append('<div class="mensaje-autor"><img src="imagenes/avatar_client.jpg" alt="" class="foto"><div class="flecha-izquierda"></div><div class="contenido">Ejecutivo : '+data+'</div></div>');
	  var elem = document.getElementById('messages');
  	  elem.scrollTop = elem.scrollHeight;
    });
    conn.on('close', function(err){ alert('Ejecutivo ha dejado el chat.');$('#messages').empty().append('');$('#chat_area').hide();$('#step2').show();$('#step3').hide(); });
  }
  $(document).ready(function() {
	...
	$('#send').live('click', function(){ 
      var msg = $('#text').val();
      conn.send(msg);
      $('#messages').append('<div class="mensaje-amigo"><div class="contenido">Yo : '+msg+'</div><div class="flecha-derecha"></div><img src="imagenes/avatar_user.jpg" alt="" class="foto"></div>');
      $('#text').val('');
	  var elem = document.getElementById('messages');
  	  elem.scrollTop = elem.scrollHeight;
    });
