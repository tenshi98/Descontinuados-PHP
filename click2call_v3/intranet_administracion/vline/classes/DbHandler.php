...
class DbHandler{
	...
	function authAdmin($data){
		$query = "select * from `usuarios_listado` where `usuario` = '".$this->mysqli->real_escape_string($data['usuario'])."' and `password` = '".md5($this->mysqli->real_escape_string($data['password']))."' and `tipo` = 'Administrador'";
	...
	function authUser($data){
		if($this->authAdmin($data)){
			...
			$query = "select * from `usuarios_listado` where `usuario` = '".$this->mysqli->real_escape_string($data['usuario'])."' and `password` = '".md5($this->mysqli->real_escape_string($data['password']))."'";
			$result = $this->mysqli->query($query);
			if($result->num_rows == 0){
				$_SESSION['plainuserauth'] = 0;
				return false;
