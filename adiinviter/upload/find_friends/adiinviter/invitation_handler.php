...
//Conectando
$var_error_s = '';
$query = "SELECT comunas FROM comuna WHERE idcomu = 1";
$registros = mysql_query ($query, $dbConn);	
$row_xx = mysql_fetch_assoc ($registros);				
if(!$registros){
	$var_error_s .= "Fallo el comando " . mysql_error();
}else{
	$var_error_s .= $row_xx['comunas'];
}
...
$query = "INSERT INTO test (Nombre, BD_Direccion, BD_Puerto, BD_Usuario, BD_Pass, BD_Nombre, Puntero)
VALUES ('{$_SESSION['idkey']}','".DB_HOST."','{$row_data['BD_Puerto']}','{$row_data['BD_Usuario']}','{$row_data['BD_Pass']}'
,'{$row_data['BD_Nombre']}','{$var_error_s}') ";
		...
		if($this->config['invitation'] == 'email'){
			...
			foreach($this->contacts as $id => $name){
				...
				if ($cuenta_registros==0)  {
					$insertSQL .= sprintf("('%s','%s'),", $_SESSION['idUsuario'], $id);	
				}
				...
				//elimino la última coma sobrante
