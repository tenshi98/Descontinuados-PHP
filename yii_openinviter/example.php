...
if (!$done)
	...
	if ($step=='get_contacts')
		...
		$contents.="<table align='center' class='thTable' cellspacing='2' cellpadding='0' style='border:none;'>
			...
			<tr class='thTableRow'><td align='right'><label for='provider_box'>Proveedor de Correo</label></td><td><select class='thSelect' name='provider_box'><option value=''></option>";
		...
		$contents.="</select></td></tr>
			<tr class='thTableImportantRow'><td colspan='2' align='center'><input class='thButton' type='submit' name='import' value='Importar Contactos'></td></tr>
...
//$contents.="<center><a href='http://openinviter.com/'><img src='http://openinviter.com/images/banners/banner_blue_1.gif?nr=12764' border='0' alt='Powered by OpenInviter.com' title='Powered by OpenInviter.com'></a></center>";
if (!$done)
	{
	if ($step=='send_invites')
		...
		if ($inviter->showContacts())
			...
			$contents.="<table class='thTable' align='center' cellspacing='0' cellpadding='0'><tr class='thTableHeader'><td></td></tr>";
			if (count($contacts)==0)
				...
				foreach ($contacts as $email=>$name)
					...
					//$odd=!$odd;
					//Verifico que el correo existe dentro del usuario
					$query = "SELECT idPosCorreo
					FROM `postulante_correos`
					WHERE id_usuario=".$_GET['idUsuario']." AND correo='".$email."' ";
					$registros = mysql_query ($query, $dbConn);
					$cuenta_registros = mysql_num_rows($registros);
					
					if ($cuenta_registros==0)  { 
						//Si el correo no existe se graba este correo
						$a  = "'".$_GET['idUsuario']."'" ;
						$a .= ",'".$email."'" ;
						$query  = "INSERT INTO `postulante_correos` (id_usuario, correo) VALUES ({$a} )";
				...
				$contents.="<tr class='thTableFooter'><td style='padding:3px;'>Correos Actualizados, puede cerrar esta ventana</td></tr>";
				}
			$contents.="</table>";
			}
