...
$original = "mnt_ubicacion_villa.php";
$location = $original;
...
if ( !empty($_POST['submit']) )  { 
	...
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_common/mnt_ubicacion_villa.php';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_controller/mnt_ubicacion_villa_1.php';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_insert/mnt_ubicacion_villa.php';
}
...
if ( !empty($_POST['submit_edit']) )  { 
	...
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_common/mnt_ubicacion_villa.php';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_controller/mnt_ubicacion_villa_1.php';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_update/mnt_ubicacion_villa.php';
}
//se borra un dato
if ( !empty($_GET['del']) )     {
	//Se consiguen las variables de busqueda y paginacion
	...
	require_once '../AA2D2CFFDJFDJX1/xrxs_form_del/mnt_ubicacion_villa.php';
}
...
<html class="no-js"><!-- InstanceBegin template="/Templates/administrador.dwt" codeOutsideHTMLIsLocked="false" -->
  <head>
    ...
    <title>Mantencion de Villas</title>
    <!-- InstanceEndEditable -->
      ...
      <div id="top">
        ...
        <header class="head">
          <div class="main-bar">
            ...
            <i class="fa fa-dashboard"></i> Mantencion de Villas
...
<div id="txf_03" class="alert_sucess">  
	Villa Modificada correctamente
	<a class="closed_c" href="javascript:void(0);" onClick="document.getElementById(&apos;txf_03&apos;).className = &apos;oculto&apos;">
		<i class="fa fa-times"></i>
	</a>
</div>
...
function cambia_ciudad(){
	...
	document.form1.idComuna.options[0].selected = true
			...
			<div class="form-group">
            	<input type="hidden" value="<?php echo $_GET['id']; ?>" name="idVilla">
...
if (isset($_GET['search'])){
	$z="WHERE mnt_ubicacion_villa.Nombre LIKE '%{$_GET['search']}%'";	
} else {
	$z="";
}
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT mnt_ubicacion_villa.idVilla FROM `mnt_ubicacion_villa` ".$z."";
...
FROM `mnt_ubicacion_villa`
LEFT JOIN `mnt_ubicacion_ciudad`    ON mnt_ubicacion_ciudad.idCiudad    = mnt_ubicacion_villa.idCiudad
LEFT JOIN `mnt_ubicacion_comunas`   ON mnt_ubicacion_comunas.idComuna   = mnt_ubicacion_villa.idComuna
".$z."
ORDER BY mnt_ubicacion_ciudad.Nombre ASC, mnt_ubicacion_comunas.Nombre ASC, mnt_ubicacion_villa.Nombre ASC
...
<label class="control-label col-lg-4">Buscar Villa</label>
    <div class="col-lg-5">
...
<?php if ($rowlevel['level']>=3){?><a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width" >Crear Nueva Villa</a><?php } ?>
</div>
...
<div class="col-lg-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Villas</h5>
		</header>
    ...
    <?php foreach ($arrComunas as $comunas) { ?>
    	<tr class="odd">
			<td class=" "><?php echo $comunas['idVilla']; ?></td>
			<td class=" "><?php echo $comunas['nombre_ciudad']; ?></td>
            <td class=" "><?php echo $comunas['nombre_comuna']; ?></td>
            <td class=" "><?php echo $comunas['nombre_villa']; ?></td>
			<td class=" ">
<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$comunas['idVilla']; ?>" title="Editar Informacion" class="icon-editar info-tooltip"></a><?php } ?>
<?php if ($rowlevel['level']>=4){?><?php $ubicacion=$location.'&del='.$comunas['idVilla'];?>
