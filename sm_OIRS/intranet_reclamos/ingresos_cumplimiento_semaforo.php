...
$original = "ingresos_cumplimiento_semaforo.php";
$location = $original;
...
// Se trae un listado con todos los departamentos
$arrDepto = array();
...
mysql_free_result($resultado);
//Se traen los dias para cada semaforo
$query = "SELECT Verde, Amarillo, Rojo
FROM `mnt_oirs_cumplimiento`
WHERE idCumplimiento = 1 ";
$resultado = mysql_query ($query, $dbConn);
$semaforo = mysql_fetch_assoc ($resultado);
?>
...
<div class="col-lg-12">
	...
	$d_cg = ($subTime_cg/(60*60*24))%365;	
    	...
    	<tr class="odd">
            ...
            <?php 
			$i = 0   ;//$oirs['dias_disponibles'];
			if($i>$semaforo['Verde']){
				echo "verdei ";
			}elseif($i<=$semaforo['Verde']&&$i>=$semaforo['Amarillo']){
			echo "verdei ";	
			}elseif($i<=$semaforo['Amarillo']&&$i>=$semaforo['Rojo']){
			echo "amarillo ";	
			}elseif($i<$semaforo['Rojo']){
				echo "rojod";
			}?> 
            "></div></td>
			<td class=" ">
...
<?php if ($rowlevel['level']>=1){?><a target="new" href="<?php echo 'view_oirs.php?view='.$oirs['id_Oirs']; ?>" title="Ver Informacion" class="icon-ver info-tooltip"></a><?php } ?>         
	...
	?>	
    	<tr class="odd">
			<td><?php echo n_doc($oirs['id_Oirs']); ?></td>
            <td><?php echo $oirs['Fecha']; ?></td>
			<td><?php echo $oirs['departamento']; ?></td>
            <td><?php echo $oirs['estado']; ?></td>
			<td><?php echo $oirs['dias_disponibles']; ?> dias</td>
			<td><?php echo $d_cg.' dias'; ?></td>
            <td><div class="semaforo 
            <?php 
			$i = 0   ;//$oirs['dias_disponibles'];
			if($i>$semaforo['Verde']){
