...
$query = "SELECT idPagListado FROM `paginas_listado` WHERE idPagCat={$_GET['filtercat']}";
...
$query = "SELECT Nombre FROM `paginas_categorias` WHERE idPagCat={$_GET['filtercat']}";
$resultado = mysql_query ($query, $dbConn);
...
$('#resultados_1').load("list_paginas_autoload_1.php<?php echo $w.'&cant_reg='.$cant_reg.$xyz; ?>", {'group_no_1':track_load_1}, function() {track_load_1++;}); //load first group
...
<?php }?>
    ...
    <td width="80%">
    <h1 class="<?php echo $config_app['title2_size'].' '.$config_app['title2_color'] ?>"><?php echo $row_data['Nombre'] ?></h1>
    </td>
  </tr>
  <tr height="90%">
    <td style="padding: 2%;" colspan="2">
...
$z='';
if(isset($_GET['filterGroup'])&&$_GET['filterGroup']!=''){
$z=" WHERE idPagGrupo={$_GET['filterGroup']}";	
}
// Se trae un listado con todos los usuarios
$arrCategorias = array();
...
<div class="content <?php echo $config_app['noti_separator'] ?>" >
	<ul id="resultados_1">
        <?php 
            ...
            echo '<a class="'.$config_app['btn_enlace_color_fondo'].' '.$config_app['btn_enlace_radio'].' '.$config_app['btn_enlace_color_texto'].' '.$config_app['btn_enlace_sombra'].' btn_link fright" href="'.$location.'&filtercat='.$categorias['idPagCat'].'">Ver</a>';  
