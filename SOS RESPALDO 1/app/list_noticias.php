...
$query = "SELECT idNotListado FROM `noticias_listado` WHERE idNotCat={$_GET['filtercat']}";
...
$query = "SELECT Nombre FROM `noticias_categorias` WHERE idNotCat={$_GET['filtercat']}";
$resultado = mysql_query ($query, $dbConn);
	   ...
	   //Modifica el tamaño de la ventana
        if($(window).scrollTop() + $(window).height() == $(document).height()) {  //user scrolled to bottom of the page?
            ...
            if(track_load_1 <= total_groups_1 && loading_1==false) {//there's more data to load
                ...
                $.post('list_noticias_autoload_1.php<?php echo $w.'&cant_reg='.$cant_reg.$xyz; ?>',{'group_no_1': track_load_1}, function(data){              
...
$z='';
if(isset($_GET['filterGroup'])&&$_GET['filterGroup']!=''){
$z=" WHERE idNotGrupo={$_GET['filterGroup']}";	
}
// Se trae un listado con todos los usuarios
$arrCategorias = array();
...
<div class="content <?php echo $config_app['noti_separator'] ?>" >
	<ul id="resultados_1">
        <?php 
            ...
            echo '<a class="'.$config_app['btn_enlace_color_fondo'].' '.$config_app['btn_enlace_radio'].' '.$config_app['btn_enlace_color_texto'].' '.$config_app['btn_enlace_sombra'].' btn_link fright" href="'.$location.'&filtercat='.$categorias['idNotCat'].'">Ver</a>';  
