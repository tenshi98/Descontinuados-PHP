...
jQuery(function($) {
	...
	uMarker = $.addMarker({
		...
		icon: 'map_img/icon/punto_mid.png',
		//draggable:true,
		events: {
			'dragend': function() {
				$.getGeocode(this);
	...
	var defaultMarker = {
		bus: 'map_img/icon/bus.png',
		bip: 'map_img/icon/bip.png'
	}
	...
	var template = {
		list: '<li>' +
			'<div class="li">' +
			'<span class="parada">' +
			'<img alt="Parada" src="map_img/parada.png"/>' +
			'</span>' +
			'<div>' +
			'<strong>{data.num!=undefined?"Parada "+data.num+" - ":""}{data.name}</strong>' +
			'</div>' +
			'<div>' +
			'<span class="small min">Código de parada: {data.cod}</span>' +
			'</div>' +
			'</div>' +
			'</li>',
		marker: {
			base: '<div id="paraderos" data-cod="{data.id}">' +
				//'<span class="close">&nbsp;</span>' +
				//'<a class="volver" href="#" title="Volver">&lsaquo; volver</a>' +
				'<span class="indi_arrow"></span>' +
				'<div class="box_parada">' +
				'<div class="w">' +
				'<header>' +
				'<span class="logoBus">&nbsp;</span>{ if (data.num!=undefined) "<span class=\'numParada\'>"+data.num+"</span>"}' +
				'<h1>{data.name}</h1>' +
				'<h2>{data.comuna}</h2>' +
				'</header>' +
				'<div class="list-wrapper">' +
				'<div class="loader">' +
				'<img src="map_img/ajax-loader2.gif" alt="Cargando..." />' +
				'</div>' +
				'<div class="c scrollable">' +
				'<div id="listado-paradas">' +
				'<ul class="listParadas">' +
				'{var tmp=\'\'; for (var servicio in data.servicios) tmp+=info(\'marker.service\',data.servicios[servicio]); tmp}' +
				'</ul>' +
				'</div>' +
				'</div>' +
				'</div>' +
				'<footer>' +
				'<p>Código de Parada: {data.cod}</p>' +
				'</footer>' +
				'</div>' +
				'</div>' +
				'</div>',
			bip: '<div class="box_datoPunto boxWhite">' +
				'<div class="ww">' +
				'<div class="header">' +
				'<div class="line">' +
				'<i class="ico ico_bip"></i>' +
				'<p>' +
				'<strong class="strong">{data.name}</strong><br/>' +
				'{data.direccion}, {data.comuna}' +
				'</p>' +
				'{data.horarios?\'<hr class="border" /><p>\':\'\'}' +
				'{var tmp="";for (var a in data.horarios) tmp+="<br/>"+data.horarios[a];tmp}' +
				'{data.horarios?\'</p>\':\'\'}' +
				'</div>' +
				'</div>' +
				'</div>' +
				'</div>',
			recorrido: '<div id="data-recorrido">' +
				'<div class="head-rec">' +
				'<span style="background-color: {data.negocio.color};" class="busColor"><img alt="Bus" src="map_img/logo_bus.png"></span>' +
				'<h1>Recorrido {data.cod}</h1>' +
				'<h2>Hacia: {data.destino}</h2>' +
				'<h3>{data.negocio.nombre}</h3>' +
				'</div>' +
				'<div class="c">' +
				'<ul>' +
				'<li class="tiempo"><i class="ico">&nbsp;</i>' +
				'<strong>Horarios</strong>{var tmp="";for (var a in data.horarios) tmp+="<br/>"+data.horarios[a];tmp}' +
				'</li>' +
				'{ if ($(\'#recorrido\').val() != data.cod) "<li class=\'recorrido\'><i class=\'ico\'>&nbsp;</i><a title=\'Ver recorrido en el mapa\' href=\'#\'>Ver recorrido2</a> en el mapa</li>"}' +
				'</ul>' +
				'</div>' +
				'</div>',
			service: '<li>' +
				'<a href="#" title="Detalles del recorrido" data-id="{data.id}" data-cod="{data.cod}" data-direction="{data.destino}">' +
				'<strong>{data.cod}</strong>' +
				'<span>{data.destino}</span>' +
				'<div class="triangle" style="border-color: transparent {data.negocio.color} transparent transparent;"></div>' +
				'</a>' +
				'</li>'
		}
	}
	...
	var showMarcadores = function(data) {
		...
		for (var a in data) {
			...
			if (data[a].type == 0) {
				...
				tmp.info(info('marker.base', (data[a])), true, {
					disableAutoPan: false
					//,maxWidth: 285
					,
					pixelOffset: new google.maps.Size(-142, -435) //-435=3 -363=2 -291=1
					//,zIndex: null
					,
					boxStyle: {
						width: "285px"
