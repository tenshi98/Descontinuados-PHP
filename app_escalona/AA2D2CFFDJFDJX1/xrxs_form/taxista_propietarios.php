...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)&&isset($Apellido_Paterno)&&isset($Apellido_Materno)){
				$sql_usuario = mysql_query("SELECT Nombre FROM taxista_propietarios WHERE Nombre='".$Nombre."' AND Apellido_Paterno='".$Apellido_Paterno."' AND Apellido_Materno='".$Apellido_Materno."' "); 
			...
			if(isset($Rut)){
				$sql_email = mysql_query("SELECT Rut FROM taxista_propietarios WHERE Rut='".$Rut."'  ");
			...
			if(isset($NombreEmpresa)){
				$sql_email = mysql_query("SELECT NombreEmpresa FROM taxista_propietarios WHERE NombreEmpresa='".$NombreEmpresa."'  ");
				$n2 = mysql_num_rows($sql_email);
			} else {$n2=0;}
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `taxista_propietarios` SET ".$a." WHERE idPropietario = '$idPropietario'";
				$result = mysql_query($query, $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `taxista_propietarios` WHERE idPropietario = {$_GET['del']}";
			$result = mysql_query($query, $dbConn);	
