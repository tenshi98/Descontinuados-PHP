...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			case 'idVista':    if(empty($idVista)){    $error['idVista']    = 'error/No ha ingresado el id del sistema';}break;
			...
			case 'Hora':       if(empty($Hora)){       $error['Hora']       = 'error/No ha ingresado la razon social';}break;
			case 'Longitud':   if(empty($Longitud)){   $error['Longitud']   = 'error/No ha ingresado el Longitud';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `robos_listado_avistados` (idRobo, idCliente, Fecha, Hora, Longitud, Latitud, Gsm, vista) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `robos_listado_avistados` SET ".$a." WHERE idVista = '$idVista'";
				$result = mysql_query($query, $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `robos_listado_avistados` WHERE idVista = {$_GET['del']}";
			$result = mysql_query($query, $dbConn);	
